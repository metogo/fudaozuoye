...
exports.WELCOME_DURATION = exports.WELCOME_STORAGE_KEY = void 0;
exports.welcomePlacement = welcomePlacement;
exports.welcomeKeyframes = welcomeKeyframes;
exports.createWelcomeVisitGuard = createWelcomeVisitGuard;
exports.WELCOME_STORAGE_KEY = "learning:welcome:nod:v1";
exports.WELCOME_DURATION = 2100;
function welcomePlacement(hero, title, dock, viewport) {
    ...
    if (hero.width < 128 || dock.width <= 0 || dock.top < 0)
        return null;
    ...
    if (y < dock.top + dock.height + 12 || y + size > viewport.height)
        return null;
    return { x, y, size, dockX: dock.left, dockY: dock.top, scale: dock.width / size };
}
function welcomeKeyframes(p) {
    const transform = (x, y, scale, angle = 0) => `translate3d(${x}px, ${y}px, 0) scale(${scale}) rotate(${angle}deg)`;
    return [
...
function createWelcomeVisitGuard(getStorage) {
    let seen = false;
    return {
        hasSeen() {
            if (seen)
                return true;
            try {
                return getStorage().getItem(exports.WELCOME_STORAGE_KEY) === "1";
            }
            catch {
                return false;
            }
        ...
        markSeen() {
            seen = true;
            try {
                getStorage().setItem(exports.WELCOME_STORAGE_KEY, "1");
            }
            catch { /* Optional enhancement, never block the app. */ }
        },
    };
}
