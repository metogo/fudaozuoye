...
function parseIllustrationStoryboard(value, solutionEvidence) {
    ...
    const frames = value.frames.map((raw) => {
        ...
        if (!containsGroundedEvidence(solutionEvidence, calculationEvidence))
...
function boundedEvidence(value) {
    if (typeof value !== "string")
        throw new Error("演算依据不合法：必须是单个字符串");
    if (value.length > 500)
        throw new Error("演算依据不合法");
    const text = value.normalize("NFC").trim();
    if (text.length < 2 || text.length > 500 || /[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(text))
        throw new Error("演算依据不合法");
    ...
    if (generic.test(normalized) || (normalized.length < 6 && !shortFormula && !shortAction))
        throw new Error("演算依据过短，不能表达有效步骤");
    return text;
}
function normalizedEvidence(value) {
    return value.normalize("NFC").replace(/\s+/g, "");
}
function containsGroundedEvidence(source, evidence) {
    ...
    while (offset >= 0) {
        ...
        if ((!startsWithDigit || !/[0-9０-９]/.test(before)) && (!endsWithDigit || !/[0-9０-９]/.test(after)))
            return true;
