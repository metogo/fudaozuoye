...
function normalizedAnswerMath(value) {
    let normalized = value.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹]+/g, (digits) => `^${Array.from(digits).map((digit) => "⁰¹²³⁴⁵⁶⁷⁸⁹".indexOf(digit)).join("")}`).normalize("NFKC")
        .replace(/\\char\s*(?:"([0-9a-f]+)|'([0-7]+)|([0-9]+))/gi, (source, hexadecimal, octal, decimal) => decodeTeXCharacter(source, hexadecimal, octal, decimal))
    ...
    return normalized
        ...
        .replace(/\^\{([^{}]+)\}/g, "^$1")
        .replace(/(-?\d+(?:\.\d+)?)\/(-?\d+(?:\.\d+)?)/g, (_, numerator, denominator) => Number(denominator) === 0 ? `${numerator}/${denominator}` : String(Number(numerator) / Number(denominator)))
        .replace(/\\(?:left|right|,|;|!|quad|qquad)/g, "")
        .replace(/[${}\s，。；：、“”‘’（）()\[\]【】]/g, "")
        .toLowerCase();
}
function decodeTeXCharacter(source, hexadecimal, octal, decimal) {
...
function shortProtectedAnswerLeak(visibleText, answer, sourceText) {
    ...
    if (phrasePatterns.some((pattern) => Array.from(visible.matchAll(pattern)).some((match) => !source.includes(match[0]))))
        return true;
    if (decoratedMathAssignmentLeak(visibleText, answer, sourceText))
        return true;
...
function decoratedMathAssignmentLeak(visibleText, answer, sourceText) {
    ...
    return expressions.some((expression) => {
        ...
        if (normalizedExpression === candidate)
            return true;
        const introducedExpression = expression.length < 3 || !sourceText.includes(expression);
        if (introducedExpression && decoratedMathPayloads(expression).some((payload) => normalizedAnswerMath(payload) === candidate))
            return true;
...
function decoratedMathPayloads(value) {
    ...
    for (const match of value.matchAll(/\\([A-Za-z]+)\*?(?:\[[^\]]*\])*\s*\{([^{}]*)\}(?:\[[^\]]*\])*\s*\{([^{}]*)\}/g)) {
        if (!semantic.has(match[1]))
            payloads.push(match[2], match[3]);
    }
    for (const match of value.matchAll(/\\([A-Za-z]+)\*?(?:\[[^\]]*\])*\s*\{([^{}]*)\}/g)) {
        if (!semantic.has(match[1]))
            payloads.push(match[2]);
    }
...
function addNumericEquivalents(value, variants) {
    const fraction = value.match(/^(-?\d+(?:\.\d+)?)\/(-?\d+(?:\.\d+)?)$/);
    if (fraction && Number(fraction[2]) !== 0)
        variants.add(String(Number(fraction[1]) / Number(fraction[2])));
