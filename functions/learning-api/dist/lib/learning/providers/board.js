...
exports.boardLessonSystemPrompt = exports.boardLessonPrompt = exports.boardCoreContentSystemPrompt = void 0;
...
var board_prompts_1 = require("./board-prompts");
Object.defineProperty(exports, "boardCoreContentSystemPrompt", { enumerable: true, get: function () { return board_prompts_1.boardCoreContentSystemPrompt; } });
Object.defineProperty(exports, "boardLessonPrompt", { enumerable: true, get: function () { return board_prompts_1.boardLessonPrompt; } });
Object.defineProperty(exports, "boardLessonSystemPrompt", { enumerable: true, get: function () { return board_prompts_1.boardLessonSystemPrompt; } });
...
function parseBoardContent(value, session, suggestion, context = []) {
    ...
    if (!Array.isArray(value.blocks) || value.blocks.length < 2 || value.blocks.length > 6)
        throw new Error("板书必须包含 2 到 6 个有明确职责的教学区块");
...
function recoverBoardContentPlan(value, session, suggestion, context = [], scope) {
    if (!Array.isArray(value.blocks) || value.blocks.length < 2 || value.blocks.length > 6)
        throw new Error("学科原生板书必须包含 2 到 6 个教学动作");
    const profile = (0, board_subject_engine_1.subjectBoardProfileFor)(session);
    const requestedMoves = value.blocks.map((block) => block && typeof block === "object" && !Array.isArray(block) ? block.move : undefined);
    ...
    if (selectedMoves.some((move) => !move))
        throw new Error("增强板书包含当前学科蓝图之外的动作");
    const indexes = selectedMoves.map((move) => profile.moves.indexOf(move));
    if (new Set(indexes).size !== indexes.length || indexes.some((index, position) => position > 0 && index <= indexes[position - 1]))
        throw new Error("增强板书必须遵循当前学科动作顺序且不能重复");
    if (scope) {
        (0, board_director_1.assertDirectedBoardMoves)(session, scope, context, requestedMoves);
    }
    const trustedEvidenceSources = [(0, problem_evidence_1.problemEvidenceText)(session.problem), ...session.nodes.flatMap((node) => node.kind === "concept" && node.diagnosticEvidence ? [node.diagnosticEvidence] : [])];
    ...
    const signatures = blocks.map((block, index) => (0, board_content_contract_1.enhancedBoardInstructionSignature)(String(block.content), selectedMoves[index].purpose, String(block.evidence)));
    if (new Set(signatures).size !== signatures.length)
        throw new Error("增强板书不同动作不能复用同一段学科套话");
    const sceneSources = blocks.map((block) => block.sourceMessageIds);
    const sourceMessageIds = [...new Set(sceneSources.flat())];
    const plan = {
        version: 2, contentRevision: 2, subject: (0, board_native_fallback_1.inferBoardSubject)(session), discipline: session.problem.subject,
        ...
        scenes: selectedMoves.map((candidate, index) => {
            ...
            return ({
                ...
                evidence: String(blocks[index].evidence), why: (0, board_native_fallback_1.nativeMoveWhy)(move.label, move.role), selfCheck: move.selfCheck,
...
function intentForRole(role) {
    ...
    if (role === "reason")
        return "derive";
    if (role === "misconception" || role === "transfer")
        return "compare";
    return "verify";
}
...
function createInstantBoardLesson(session, scope, suggestion, context = []) {
    try {
...
function buildNativeBoardLesson(session, scope, suggestion, quality, context = []) {
    const allBlocks = (0, board_native_fallback_1.createNativeBoardBlocks)(session, scope);
    const selectedIds = new Set((0, board_director_1.directBoardBlueprint)(session, scope, context).map((move) => move.id));
    const profile = (0, board_subject_engine_1.subjectBoardProfileFor)(session);
    ...
    const annotations = blocks.slice(0, 3).map((block, index) => annotation(block, "", kinds[index], reasons[index]));
...
function createMinimalSubjectBoardLesson(session, suggestion, degradedReason) {
    const profile = (0, board_subject_engine_1.subjectBoardProfileFor)(session);
    const selectedMoves = (0, board_director_1.directBoardBlueprint)(session, { kind: "problem" });
    ...
    const blocks = selectedMoves.map((move, index) => ({
        id: `board-${index + 1}`,
...
function minimalSubjectInstruction(subject) {
    const instructions = {
...
function assertNoAnswerLeak(session, title, blocks, annotations, visual, plan) {
    ...
    if ((0, answer_protection_1.explicitAnswerClaimLeak)(visibleText, rawAnswer))
        throw new Error("板书不能提前泄露原题最终答案");
    if ((0, answer_protection_1.shortTextAnswerLeak)(visibleText, rawAnswer, problemEvidence))
        throw new Error("板书不能提前泄露原题最终答案");
