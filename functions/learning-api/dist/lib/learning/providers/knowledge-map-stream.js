    ...
    const worker = async () => {
        try {
            while (!failed && cursor < plan.nodes.length) {
                ...
                const edges = await (0, knowledge_map_validation_1.requestMapValue)(request, relationInstruction, JSON.stringify({ problem: JSON.parse(prompt), node: { title: concepts[index].title, evidence: concepts[index].evidence }, parents: node.parents.map(id => { const parent = concepts.find(n => n.id === id); return { title: parent.title, evidence: parent.evidence }; }) }), value => (0, knowledge_map_validation_1.validateMapRelations)(value, node), knowledge_map_deadline_1.MAP_NODE_TIMEOUT_MS);
                if (failed)
                    return;
