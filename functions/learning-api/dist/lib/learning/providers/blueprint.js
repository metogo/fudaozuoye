...
exports.parseProblemGuide = parseProblemGuide;
exports.parseKnowledgeSelections = parseKnowledgeSelections;
exports.repairSelectionReasonGrounding = repairSelectionReasonGrounding;
...
function parseProblemGuide(value, problemText, evidenceQuotes = [], originalAnswer = "", originalExplanation = "", safeFallback) {
    ...
    Object.entries(parsed).forEach(([key, content]) => (0, presentation_1.assertBalancedLearningMarkup)(content, `原题引导 ${key}`));
    if (parsed.goal.length < 8 || parsed.keyClue.length < 8 || parsed.approach.length < 12 || parsed.firstQuestion.length < 6)
        throw new Error("原题引导清理后内容不完整");
...
function spokenNumberVariant(value) {
...
function numberToChinese(value) {
    ...
    let result;
    if (!Number.isInteger(numeric) || numeric < 0 || numeric > 9_999)
        result = [...integer].map((item) => digits[Number(item)] ?? item).join("");
    else if (numeric < 10)
        ...
        [...String(numeric)].forEach((item, index, all) => {
            ...
            if (digit === 0)
                ...
                if (pendingZero)
                    parts.push("零");
                if (!(digit === 1 && unit === "十" && parts.length === 0))
    ...
    if (decimal)
        result += `点${[...decimal].map((item) => digits[Number(item)] ?? item).join("")}`;
    return `${negative ? "负" : ""}${result}`;
}
function groundedExcerpt(problemText) {
...
function clueScore(value) {
    return Number(/[=<>≤≥+×÷/]|\d/.test(value)) * 10 + Math.min(value.length, 50) / 50;
}
function containsCompleteAnswer(guideText, answer) {
...
function sharesContinuousPhrase(source, target, size) {
    const normalizedSource = normalize(source);
    const normalizedTarget = normalize(target);
    if (normalizedSource.length < size || normalizedTarget.length < size)
        return false;
    for (let index = 0; index <= normalizedSource.length - size; index += 1) {
        if (normalizedTarget.includes(normalizedSource.slice(index, index + size)))
            return true;
    }
    return false;
}
function parseKnowledgeSelections(result, options) {
    ...
    const selections = rawSelections.map((raw, index) => {
        ...
        if (draft.simplification.length < 12)
            throw new Error(`节点 ${draft.conceptId} 的简化理由清理后内容不完整`);
...
function repairSelectionReasonGrounding(result) {
    ...
    return {
        ...
        [key]: selections.map((raw) => {
            ...
            if (typeof conceptId !== "string" || typeof evidence !== "string" || typeof simplification !== "string")
                return raw;
            if (normalize(simplification).includes(normalize(evidence)))
                return raw;
...
function parseBlueprint(value) {
    ...
    ].forEach(([label, content]) => (0, presentation_1.assertBalancedLearningMarkup)(content, label));
    return {
        conceptId,
        evidence,
        evidenceSource,
...
function cleanUserFacingModelText(value) {
    return value
