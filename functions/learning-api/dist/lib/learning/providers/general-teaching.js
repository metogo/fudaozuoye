...
exports.generalTeachingSystem = void 0;
exports.assembleGeneralLesson = assembleGeneralLesson;
exports.generateGeneralTeaching = generateGeneralTeaching;
const problem_evidence_1 = require("../problem-evidence");
const illustration_fingerprint_1 = require("../illustration-fingerprint");
const teaching_verifier_1 = require("../teaching-verifier");
const teaching_scene_1 = require("../teaching-scene");
const teaching_audit_1 = require("../teaching-audit");
exports.generalTeachingSystem = `你是跨学科教学演示编排器。任意题目都使用同一个数据协议，不选择题型模板。
...
function assembleGeneralLesson(session, p, evaluated) {
    const frames = p.steps.map((step, index) => {
        ...
        const point = (p) => [xoffset + (p[0] - xmin) * xscale, 520 - yoffset - (p[1] - ymin) * yscale];
        const shapes = [];
        for (const o of evaluated[index].objects) {
            const points = o.points.map(point);
            if (points.flat().some(v => !Number.isFinite(v) || Math.abs(v) > 4000))
                throw Error("图形超出可读范围，请调整坐标范围");
            const color = o.color ?? "outline";
            if (o.kind === "axes") {
                if (ymin <= 0 && ymax >= 0)
                    shapes.push({ kind: "path", id: `${o.id}_x`, points: [point([xmin, 0]), point([xmax, 0])], arrow: true, color });
                if (xmin <= 0 && xmax >= 0)
                    shapes.push({ kind: "path", id: `${o.id}_y`, points: [point([0, ymin]), point([0, ymax])], arrow: true, color });
                for (const [name, at] of [["x", [xmax, 0]], ["y", [0, ymax]], ["0", [0, 0]]]) {
                    if (at[0] >= xmin && at[0] <= xmax && at[1] >= ymin && at[1] <= ymax) {
                        const q = point([...at]);
                        shapes.push({ kind: "label", id: `${o.id}_${name}_label`, x: q[0] + 15, y: q[1] + 20, text: name });
                    }
                }
            }
            else if (o.kind === "label")
                shapes.push({ kind: "label", id: o.id, x: points[0][0], y: points[0][1], text: o.text });
            else if (o.kind === "circle" || o.kind === "point") {
                const radius = o.kind === "point" ? 4 : o.radius * scale;
                if (!(radius > 0) || radius > 2000)
                    throw Error("半径超出可读范围");
                points.forEach((p, i) => shapes.push({ kind: "circle", id: `${o.id}_${i}`, x: p[0], y: p[1], radius, color }));
            }
            else
                shapes.push({ kind: "path", id: o.id, points, closed: o.kind === "polygon", arrow: o.kind === "arrow", color });
        ...
        const refs = new Set(), pending = [...step.refs];
        const dependencies = new Map([...p.variables.map(v => [v.name, v.refs]), ...p.steps.map(s => [s.id, s.refs])]);
        while (pending.length) {
            const ref = pending.pop();
            if (refs.has(ref))
                continue;
            refs.add(ref);
            pending.push(...(dependencies.get(ref) ?? []));
        }
        const sourceQuotes = p.conditions.filter(c => refs.has(c.id)).map(c => c.quote);
        const readableShapes = shapes.flatMap(shape => {
            if (shape.kind !== "label")
                return [shape];
            const lines = [];
            let line = "", width = 0;
        ...
        const occupied = [];
        for (const shape of readableShapes)
            if (shape.kind === "label") {
                ...
                for (const offset of candidates) {
                    ...
                    if (y < 30 || y > 490 || occupied.some(b => Math.abs(b.y - y) < 32 && Math.abs(b.x - shape.x) < b.half + half + 12))
                        continue;
                    shape.y = y;
                    break;
                }
        ...
        const scene = { version: 1, template: "general", worldId: (0, illustration_fingerprint_1.illustrationFingerprint)(session), stageIds: [step.id], shapes: readableShapes, sourceQuotes };
        return { id: `frame-${index + 1}`, index: index + 1, title: step.title, calculation: step.explanation,
            ...
            alt: step.title, imageUrl: `data:image/svg+xml;base64,${Buffer.from((0, teaching_scene_1.teachingSceneSvg)(scene)).toString("base64")}`, scene,
            ...
            verification: [...evaluated[index].checks, ...(evaluated[index].inferredSymbols?.length ? [{ status: "unknown", detail: `自由符号 ${evaluated[index].inferredSymbols.join("、")} 未赋数值；不据此假设参数取值` }] : []), { status: "unknown", detail: "条件引用已核对出处；文字推理及图意匹配不等于已获数学证明" }], schematic: true, sourceQuotes };
    });
    return { version: 1, requestId: session.requestId, problemFingerprint: (0, illustration_fingerprint_1.illustrationFingerprint)(session), title: p.title, frameCount: frames.length, frames };
}
async function generateGeneralTeaching(session, request, audit, signal) {
    if ((0, problem_evidence_1.needsVisualReview)(session.problem))
        throw Error("需要补充条件：请确认题图中的关键标注，或上传更清晰的原图后重试。");
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            try {
                ...
                if (remaining <= 0 || controller.signal.aborted)
                    throw Error("演示生成超过35秒预算");
                raw = await new Promise((resolve, reject) => {
                    const stop = () => reject(new Error("演示生成已停止"));
                    controller.signal.addEventListener("abort", stop, { once: true });
                    request(exports.generalTeachingSystem, prompt, remaining, controller.signal).then(resolve, reject).finally(() => controller.signal.removeEventListener("abort", stop));
                });
                const clarification = (() => { try {
                    return JSON.parse(raw).clarification;
                }
                catch {
                    return null;
                } })();
                if (typeof clarification === "string" && clarification.trim())
                    throw new Error(`需要补充条件：${clarification.slice(0, 300)}`);
                const program = (0, teaching_verifier_1.parseTeachingProgram)(raw, evidence);
                const evaluated = await (0, teaching_verifier_1.verifyTeachingProgram)(program, controller.signal, Math.min(3000, Math.max(1, 35000 - (Date.now() - start))));
                ...
                let auditRaw;
                try {
                    auditRaw = await new Promise((resolve, reject) => {
                        ...
                        if (!auditController.signal.aborted)
                            audit(teaching_audit_1.teachingAuditSystem, (0, teaching_audit_1.teachingAuditInput)(evidence, program, evaluated, lesson), auditBudget, auditController.signal).then(resolve, reject).finally(() => auditController.signal.removeEventListener("abort", stop));
                    });
                }
                finally {
                    auditMs += Date.now() - auditStart;
                    clearTimeout(auditTimer);
                    controller.signal.removeEventListener("abort", stopAudit);
                }
                const verdict = (0, teaching_audit_1.parseTeachingAudit)(auditRaw);
                if (verdict.verdict !== "pass")
                    throw Error(`图题校对${verdict.verdict === "reject" ? "发现错配" : "不确定"}：${verdict.issues.join("；")}`);
                if (controller.signal.aborted)
                    throw Error("演示生成已停止");
                ...
                if (signal?.aborted)
                    throw new DOMException("请求已取消", "AbortError");
                if (controller.signal.aborted)
                    throw Error(`演示生成超过35秒预算，请重试${lastValidationFailure ? `（上次核验：${lastValidationFailure.slice(0, 160)}）` : ""}`);
                const message = error instanceof Error ? error.message : "协议核验失败";
                if (attempt || message.startsWith("需要补充条件") || message.includes("8秒累计预算") || message.includes("校对响应格式") || message.startsWith("图题校对不确定"))
                    throw Error(`${message}${attempt && lastValidationFailure ? `（上次核验：${lastValidationFailure.slice(0, 160)}）` : ""}`);
