...
async function generateCandidate(client, session, scope, suggestion, context) {
    ...
    if (client.protocol === "chat-completions") {
        const first = await client.toolRequest((0, board_1.boardCoreContentSystemPrompt)(learnerBand), `${prompt}\n优先使用 5 个教学单元，直接调用指定函数。`, (0, board_1.boardContentTool)(), 2400, candidateTimeoutMs);
        try {
            return (0, board_1.addSafeBoardAnnotations)((0, board_1.recoverBoardContentPlan)((0, model_support_1.parseJsonObject)(first), session, suggestion, context), session);
        }
        catch (error) {
            ...
            return (0, board_1.addSafeBoardAnnotations)((0, board_1.recoverBoardContentPlan)((0, model_support_1.parseJsonObject)(repaired), session, suggestion, context), session);
        }
    }
