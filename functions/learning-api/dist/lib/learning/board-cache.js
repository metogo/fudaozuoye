...
function isStoredSemanticVisual(value) {
    ...
    if (visual.kind === "formula_chain")
        return Array.isArray(visual.steps) && visual.steps.length >= 2 && visual.steps.length <= 6 && visual.steps.every((step) => isRecord(step) && typeof step.id === "string" && typeof step.expression === "string" && step.expression.length <= 180 && typeof step.explanation === "string" && step.explanation.length <= 80);
...
function isStoredGeometryObject(value, pointIds) {
    ...
    if (value.type === "right_angle")
        return typeof value.vertex === "string" && typeof value.from === "string" && typeof value.to === "string" && new Set([value.vertex, value.from, value.to]).size === 3 && pointIds.has(value.vertex) && pointIds.has(value.from) && pointIds.has(value.to);
