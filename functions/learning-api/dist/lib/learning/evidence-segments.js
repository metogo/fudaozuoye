...
exports.MAX_EVIDENCE_LENGTH = void 0;
exports.splitEvidence = splitEvidence;
/** Preserve verbatim evidence: never cut a formula at a newline or size limit. */
exports.MAX_EVIDENCE_LENGTH = 4000;
function splitEvidence(source, targetLength = 220) {
    const segments = [];
    let start = 0;
    let delimiter = "";
    let braces = 0;
    ...
    const append = (end) => {
        ...
        if (text.length > exports.MAX_EVIDENCE_LENGTH)
            throw new Error("单条题目公式过长，无法完整引用");
        if (text)
            segments.push(text);
        start = end;
    };
    for (let i = 0; i < source.length; i++) {
        ...
        if (!delimiter && braces === 0 && (/[。；;\n]/.test(source[i]) || (i + 1 - start >= targetLength && safeSizeBoundary)))
            append(i + 1);
    }
    if (delimiter || braces)
        throw new Error("原题公式结构不完整，无法完整引用");
