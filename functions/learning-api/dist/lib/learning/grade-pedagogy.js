...
exports.gradeBandLabels = exports.gradeBands = void 0;
exports.teachingBandOf = teachingBandOf;
exports.withLearnerBand = withLearnerBand;
exports.gradeTeachingInstruction = gradeTeachingInstruction;
exports.inspectGradeLanguage = inspectGradeLanguage;
exports.assertGradeLanguage = assertGradeLanguage;
exports.adaptTeachingCopy = adaptTeachingCopy;
exports.adaptBoardLessonForGrade = adaptBoardLessonForGrade;
exports.gradeBands = ["primary", "junior", "senior"];
exports.gradeBandLabels = {
    primary: "小学",
    junior: "初中",
    senior: "高中",
};
...
function teachingBandOf(problem) {
    return problem.learnerBand ?? problem.gradeBand;
}
function withLearnerBand(problem, learnerBand) {
    return { ...problem, learnerBand };
}
function gradeTeachingInstruction(band, surface) {
    const shared = `当前学生学段：${exports.gradeBandLabels[band]}。本次任务：${surfaceGoals[surface]}。保持学科事实、必要步骤与结论准确，不降低知识要求，只调整表达方式。`;
    if (band === "primary")
        return [
            shared,
...
function inspectGradeLanguage(text, band, sourceText = "", surface = "chat") {
    if (band === "senior") {
    ...
    if (band !== "primary")
        return [];
    const issues = [];
    if (primaryMetaTerms.some((term) => hasUnexplainedMetaTerm(text, sourceText, term)))
        issues.push("abstract_meta_language");
    const prose = maskSourceQuotes(text, sourceText).replace(/\$\$[\s\S]*?\$\$|\$[^$]*\$/g, "公式").replace(/^#{1,6}\s+.*$/gm, "")
        .replace(/\n(?!(?:\s*(?:[-*+]\s|\d+[.)、]\s)))/g, " ");
    const sentenceLimit = surface === "board" ? 96 : 72;
    if (prose.split(/[。！？!?.\n]/).some((sentence) => [...sentence.replace(/\s/g, "")].length > sentenceLimit && !containsSourceExcerpt(sentence, sourceText)))
        issues.push("sentence_too_long");
    const actionSentences = prose.split(/[。！？!?.\n]/).filter(Boolean);
    if (actionSentences.some(hasTooManySequentialActions))
        issues.push("too_many_actions");
    if (surface !== "solution" && surface !== "board" && text.split(/\n{2,}/).some((paragraph) => !/^\s*\d+[.)、]\s/m.test(paragraph) && countMatches(paragraph, /(?:^|[。！？!?]\s*)(?:先|再|然后|接着|最后)/g) >= 3)) {
        if (!issues.includes("too_many_actions"))
            issues.push("too_many_actions");
    }
    return issues;
}
function maskSourceQuotes(text, sourceText) {
    return sourceText.split(/[。！？!?\n]/).map((item) => item.trim()).filter((item) => item.length >= 8)
        .reduce((value, quote) => value.replaceAll(quote, "题目原文"), text);
}
function containsSourceExcerpt(text, sourceText) {
    const compactText = text.replace(/\s/g, "");
    const compactSource = sourceText.replace(/\s/g, "");
    if (compactText.length < 24 || compactSource.length < 24)
        return false;
...
function assertGradeLanguage(text, band, label, sourceText = "", surface = "chat") {
    ...
    throw new Error(`${label}不符合${exports.gradeBandLabels[band]}表达要求：${issues.map((issue) => descriptions[issue]).join("、")}`);
}
function countMatches(value, pattern) {
    return value.match(pattern)?.length ?? 0;
}
function hasTooManySequentialActions(sentence) {
...
function actionSignature(clause) {
    ...
    if (!verb)
        return /^(?:也得)?(?:把|将)/.test(raw) ? "" : compact.slice(0, 12);
    if (/圈出|找出|找到|挑出|理出/.test(verb))
        return "locate";
    if (/写出|写|列出|列式/.test(verb))
        return "form";
    if (/计算|算出|求出/.test(verb))
        return "calculate";
    if (/想清|看清/.test(verb))
        return "understand";
    return verb;
}
function hasUnexplainedMetaTerm(text, sourceText, term) {
    ...
    while (index >= 0) {
        ...
        if (!/(?:也就是|意思是|指的是|就是|可以理解为|说白了|换句话说|我们把.{0,16}叫作|称为)/.test(context))
            return true;
...
function adaptTeachingCopy(text, band) {
...
function adaptBoardLessonForGrade(lesson, band) {
    if (band === "senior")
        return lesson;
    const adapt = (value) => value === undefined ? undefined : adaptTeachingCopy(value, band);
    const adaptStructure = (value) => band === "primary" ? adaptTeachingCopy(value, band) : value;
...
function adaptOutsideProtected(text, protectedValues, band) {
    let segments = [{ text, protected: false }];
    for (const value of protectedValues.filter((item) => Boolean(item))) {
        segments = segments.flatMap((segment) => {
            if (segment.protected || !segment.text.includes(value))
                return [segment];
...
function adaptLegacyVisual(visual, band) {
    if (!visual)
        return visual;
    return { ...visual, title: adaptTeachingCopy(visual.title, band), caption: adaptTeachingCopy(visual.caption, band) };
}
function adaptBoardVisual(visual, band) {
    ...
    if (visual.kind === "concept_graph")
        return { ...visual, ...copy, nodes: visual.nodes.map((node) => ({ ...node, label: adaptTeachingCopy(node.label, band) })), edges: visual.edges.map((edge) => ({ ...edge, label: edge.label ? adaptTeachingCopy(edge.label, band) : undefined })) };
    if (visual.kind === "formula_chain")
        return { ...visual, ...copy, steps: visual.steps.map((step) => ({ ...step, explanation: adaptTeachingCopy(step.explanation, band) })) };
    if (visual.kind === "evidence_chain")
        return { ...visual, ...copy, links: visual.links.map((link) => ({ ...link, meaning: adaptTeachingCopy(link.meaning, band) })) };
    if (visual.kind === "timeline")
        return { ...visual, ...copy, events: visual.events.map((event) => ({ ...event, event: adaptTeachingCopy(event.event, band) })) };
    if (visual.kind === "process_flow")
        return { ...visual, ...copy, steps: visual.steps.map((step) => ({ ...step, label: adaptTeachingCopy(step.label, band) })) };
    if (visual.kind === "comparison_matrix")
        return { ...visual, ...copy, columns: visual.columns.map((column) => adaptTeachingCopy(column, band)), rows: visual.rows.map((row) => ({ ...row, aspect: adaptTeachingCopy(row.aspect, band), left: adaptTeachingCopy(row.left, band), right: adaptTeachingCopy(row.right, band) })) };
    return { ...visual, ...copy };
}
