...
exports.remarkConclusionLayout = remarkConclusionLayout;
const sectionTitle = /^(结论|最终答案|最后结果|答案与结论|结果与结论)$/;
const numberedAnswer = /^[（(]\d+[)）]\s*\S/;
const plainText = (nodes) => nodes.map((node) => "children" in node ? plainText(node.children) : "value" in node ? node.value : "").join("");
// Split only prose between answers. Math, code and their source stay intact.
function separateAnswers(nodes) {
...
function remarkConclusionLayout({ source }) {
    return (tree) => {
        let conclusionLevel = 0;
        tree.children = tree.children.map((node) => {
            if (node.type === "heading") {
                const title = plainText(node.children).trim().replace(/[：:]$/, "");
                if (sectionTitle.test(title)) {
                    conclusionLevel = node.depth;
                    return node;
                }
