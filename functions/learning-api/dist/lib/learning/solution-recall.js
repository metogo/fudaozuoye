...
exports.createGroundedRecallCheck = createGroundedRecallCheck;
exports.parseGroundedRecallCheck = parseGroundedRecallCheck;
...
function solutionRecallCheck(session) {
    if (session.solutionRecallCheck)
        return session.solutionRecallCheck;
    return createGroundedRecallCheck(session);
}
function createGroundedRecallCheck(session, solution) {
    ...
    const score = (line) => (/[=＝→]/.test(line) ? 6 : 0) +
        (/\d/.test(line) ? 3 : 0) +
...
function parseGroundedRecallCheck(value, session, solution) {
    const { sourceQuote, question, answer, explanation } = value;
    if ([sourceQuote, question, answer, explanation].some((part) => typeof part !== "string" || part.trim().length < 8))
        throw new Error("关键步骤检查缺少具体问题或依据");
    const quote = sourceQuote.trim();
    const prompt = question.trim();
    if (!solution.includes(quote) || quote.length > 300 || prompt.length > 240 || answer.length > 1200 || explanation.length > 1200)
        throw new Error("关键步骤未对应刚才的讲解");
    if (/题目要求哪个量|哪条条件最接近|说清楚一个关键步骤|题目要解决什么/.test(prompt))
        throw new Error("关键步骤问题不能使用通用套话");
    const check = createGroundedRecallCheck(session, solution);
    return { ...check, prompt: `刚才讲解中的这一步：\n\n> ${quote}\n\n${prompt}`, answer: `当前原题：${session.problem.text}\n当前步骤：${quote}\n参考解释：${answer}`, explanation: explanation };
}
