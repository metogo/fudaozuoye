...
exports.learningPlainText = learningPlainText;
exports.buildLearningClipboardContent = buildLearningClipboardContent;
exports.writeLearningClipboard = writeLearningClipboard;
const copy_math_text_1 = require("./copy-math-text");
...
function plainText(node, protect, listDepth = 0) {
    if (node.nodeType === 3)
        return node.textContent ?? "";
    if (node.nodeType !== 1)
        return "";
    const element = node;
    if (element.matches(omittedContent))
        return "";
    if (element.tagName === "PRE")
        return `\n\n${protect(element.textContent ?? "")}\n\n`;
    if (element.tagName === "CODE")
        return protect(element.textContent ?? "");
    if (element.matches(".katex")) {
        const formula = (0, copy_math_text_1.readableFormula)(element);
...
function learningPlainText(source) {
    let prefix = "\uE000code:";
    while (source.textContent?.includes(prefix))
        prefix += ":";
    const fragments = [];
    const protect = (text) => {
    ...
    return prose.replace(new RegExp(`${prefix}(\\d+)\uE001`, "g"), (_, index) => fragments[Number(index)]);
}
/** Export only the rendered prose; never put UI controls or KaTeX's twin renderings on the clipboard. */
function buildLearningClipboardContent(source) {
...
async function writeLearningClipboard(content) {
