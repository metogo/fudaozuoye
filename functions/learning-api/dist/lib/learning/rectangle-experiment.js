...
exports.experimentObservationLimit = exports.experimentStart = void 0;
exports.supportsRectangleExperiment = supportsRectangleExperiment;
exports.rectangleMetrics = rectangleMetrics;
exports.resizeRectangle = resizeRectangle;
exports.experimentQuestion = experimentQuestion;
exports.experimentStart = { length: 4, width: 2 };
// 留出实验事实和边界说明的空间，完整追问须满足现有接口的 300 字限制。
exports.experimentObservationLimit = 100;
/** Deliberately narrow: compound shapes, missing diagrams and inferred dimensions do not qualify. */
function supportsRectangleExperiment(problem) {
    const text = problem.text.trim();
    if (problem.subject !== "math" || !Number.isFinite(problem.confidence) || problem.confidence < .85
        || problem.missingVisualInformation?.length || problem.visualContext?.affectsSolving || text.length > 300)
        return false;
    if (!/长方形|\brectangle\b/i.test(text) || !/周长|面积|\bperimeter\b|\barea\b/i.test(text))
        return false;
    if (/不是|并非|两个|两块|多个|若干|拼|剪|切|阴影|梯形|三角|正方形|圆|折|旋转|围成|比|增加|减少|扩大|缩小|倍|长方体|立方|not a|shaded|triangle|square|circle|cut|two rectangles/i.test(text))
        return false;
    ...
    if (length.length !== 1 || width.length !== 1)
        return false;
    const unit = (value) => /^(厘米|cm)$/i.test(value) ? "cm" : "m";
...
function rectangleMetrics(rectangle) {
    ...
    if (![length, width].every(n => Number.isInteger(n) && n >= 1 && n <= 8) || width > length)
        throw new Error("实验尺寸不合法");
    return { perimeter: 2 * (length + width), area: length * width };
}
function resizeRectangle(current, side, value, fixedSum) {
    rectangleMetrics(current);
    if (!Number.isInteger(value))
        throw new Error("实验尺寸必须为整数");
    if (fixedSum !== null) {
        if (fixedSum !== current.length + current.width)
            throw new Error("固定周长约束不一致");
...
function experimentQuestion(before, after, fixedSum, question) {
    if (question.length > exports.experimentObservationLimit)
        throw new Error("实验观察最多 100 字");
    const a = rectangleMetrics(before), b = rectangleMetrics(after);
    if (fixedSum !== null && (before.length + before.width !== fixedSum || after.length + after.width !== fixedSum))
        throw new Error("实验对比不满足固定周长");
