...
exports.subjectBoardProfile = subjectBoardProfile;
exports.subjectBoardProfileByMove = subjectBoardProfileByMove;
exports.allSubjectBoardMoves = allSubjectBoardMoves;
exports.subjectBoardProfileFor = subjectBoardProfileFor;
exports.createSubjectNativeBlocks = createSubjectNativeBlocks;
...
function subjectBoardProfile(subject) { return profiles[subject]; }
function subjectBoardProfileByMove(subject, firstMove) {
...
function subjectBoardProfileFor(session) {
    ...
    if (session.problem.subject === "biology" && biologyExperimentTask && biologyExpressionTask)
        return biologyExperimentProfile;
    if (session.problem.subject === "biology" && biologyExpressionTask)
        return biologyExpressionProfile;
    if (session.problem.subject === "biology" && /(?:X染色体|Y染色体|性染色体|伴性|正交|反交|性别.{0,8}遗传|遗传.{0,8}性别|果蝇.{0,8}眼色)/i.test(text))
        return biologyLinkageProfile;
    if (session.problem.subject === "biology" && biologyExperimentTask)
        return biologyExperimentProfile;
    if (session.problem.subject === "biology" && /(?:亲本|杂交|后代|配子|性状分离|显性.{0,10}隐性|基因型.{0,12}(?:比例|组合)|表现型.{0,12}(?:比例|组合)|遗传比例)/i.test(text))
        return biologyGeneticsProfile;
    ...
    if (session.problem.subject === "chemistry" && reactionTypeTask)
        return chemistryReactionTypeProfile;
    if (session.problem.subject === "chemistry" && substanceClassificationTask && (explicitClassificationIntent || !reactionProcessTask))
        return chemistryClassificationProfile;
    const vocabularyTask = /(?:词义|单词|短语含义|word meaning|meaning of (?:the )?(?:word|phrase)|what does .{1,36} mean|vocabulary)/i.test(text);
    if (session.problem.subject === "english" && vocabularyTask)
        return englishVocabularyProfile;
    ...
    if (session.problem.subject === "english" && readingEvidenceTask)
    ...
    if (session.problem.subject === "english" && grammarTask)
        return englishGrammarProfile;
    if (session.problem.subject === "physics" && /(?:小孔成像|针孔成像)/i.test(text))
        return physicsPinholeProfile;
    if (session.problem.subject === "physics" && /(?:凸透镜|凹透镜|透镜成像|放大镜|投影仪|幻灯机|照相机.{0,8}(?:镜头|成像|底片)|物距.{0,8}(?:焦距|2f|二倍焦距))/i.test(text))
        return physicsLensProfile;
    if (session.problem.subject === "physics" && /(?:折射|反射|透镜|光线|入射角|折射角|光路|成像|像的位置)/i.test(text))
        return physicsOpticsProfile;
    return profiles[session.problem.subject];
}
function createSubjectNativeBlocks(session, scope) {
...
function biologyExperimentBlocks(c) {
    return blocks(["研究问题", `${c.task} 先把问题压成“某一因素是否影响某个可观察指标”，明确研究对象，不同时研究多个变化。`], ["变量表", `${c.clue} 分成自变量、因变量和控制变量；每个条件只放进一种角色，避免把观察结果写成控制条件。`], ["对照设计", `${c.approach} 设置实验组与对照组，两组只改变研究因素，其余对象、时间、环境和测量方式保持一致。`], ["观察证据", `逐次记录可测结果并比较组间差异；“看到什么”与“为什么会这样”分开，单次偶然现象不能直接当成规律。`], ["结论边界", `${c.question} 结论只说明当前对象、条件和指标下的影响；没有重复、对照或稳定差异时，应保留判断而不是补写机制。`]);
}
function chemistryClassificationBlocks(c) {
    return blocks(["分类对象", `${c.task} 先逐项写出题目要求判断的对象，明确是在区分混合物、纯净物，还是继续区分单质与化合物。`], ["分类依据", `${c.clue} 当前只使用组成是否单一、是否由同一种物质构成等题目可核对依据，不能拿颜色、状态或名称长短代替标准。`], ["逐项归类", `${c.approach} 对每个对象分别写成“原题信息—采用标准—所属类别”，每一项独立判断，不用前一个对象的结论替代证据。`], ["边界辨析", "物质的状态、用途和俗称不能直接决定类别；同名混合物也可能组成不同，题目没有给出组成时应保留判断。"], ["分类迁移", `${c.question} 换一组物质后仍先确定分类层级，再用同一组成标准逐项判断，最后反查是否混用了不同口径。`]);
}
function chemistryReactionTypeBlocks(c) {
    return blocks(["两侧计数", `${c.task} 先按箭头分开反应物和生成物，只统计不同物质的种类，不把化学计量系数重复算成多种物质。`], ["结构骨架", `${c.clue} 把完整方程式压成“多种物质 → 一种物质”“一种物质 → 多种物质”或“多种 → 多种”的骨架。`], ["类型判定", `${c.approach} 多种反应物生成一种物质是化合反应，一种反应物生成多种物质是分解反应；“单质+化合物→新单质+新化合物”是置换反应，“两种化合物交换成分→两种新化合物”是复分解反应。`], ["概念边界", "遇到“多种→多种”必须继续核对物质身份：置换反应有单质参与并生成新单质，复分解反应则是两种化合物相互交换成分；方程式系数不改变反应类型。"], ["方程式复核", `${c.question} 回到箭头两侧逐项点数，确认没有把同一物质的系数、状态符号或反应条件误当成新的物质。`]);
}
function englishVocabularyBlocks(c) {
    return blocks(["Word evidence", `Locate the target word or phrase and keep its local sentence: “${c.evidence}”. The answer must stay inside this context.`], ["Local grammar", `${c.clue} Identify its word form, modifier and sentence role before choosing a meaning.`], ["Context clues", `${c.approach} Combine contrast, example, cause, reference and tone clues; one familiar translation is not enough.`], ["Meaning boundary", "Replace the word with each candidate meaning. Reject any option that breaks the grammar, logic or tone of the sentence."], ["Answer in context", `${c.question} State the contextual meaning, quote the decisive clue and explain why it fits this sentence.`]);
}
function biologyGeneticsBlocks(c) {
...
function biologyLinkageBlocks(c) {
...
function biologyExpressionBlocks(c) {
    const translationSite = currentAnswerIs(c, "核糖体") ? "蛋白质合成场所" : "核糖体";
    return blocks(["信息起点", `${c.task} 先把基因看作携带遗传信息的 DNA 片段，表达目标是形成具有特定结构和功能的蛋白质，而不是把基因直接变成性状。`], ["转录过程", `${c.clue} 以 DNA 的一条链为模板，按碱基互补关系合成 RNA；真核细胞中这一步主要在细胞核完成，产物携带可供读取的信息。`], ["翻译过程", `${c.approach} mRNA 到达${translationSite}后，密码子依次被读取，tRNA 携带相应氨基酸参与连接，形成具有特定顺序的多肽链。`], ["蛋白质到性状", "蛋白质通过催化、运输或构成细胞结构等功能影响生命活动，进而影响性状；不能把 DNA、RNA、蛋白质和性状写成同一种物质。"], ["表达链复核", `${c.question} 最后按“基因中的 DNA 信息 → 转录形成 RNA → ${translationSite}完成翻译并形成蛋白质 → 蛋白质功能影响性状”复核方向、场所和产物。`]);
}
function englishGrammarBlocks(c) {
...
function physicsPinholeBlocks(c) {
    return blocks(["装置与对象", `${c.task} 先按“发光或反光物体—小孔—光屏”排好装置，物距与像距都以小孔所在平面为基准。`], ["直线光路", `${c.clue} 从物体上、下两个特征点各画一条穿过小孔的直线，光线穿孔后继续直线传播并到达光屏。`], ["比例模型", `${c.approach} 小孔两侧形成相似三角形，可用 $\\frac{h'}{h}=\\frac{v}{u}$ 判断大小变化，其中 $h'$、$h$ 分别表示像高和物高，先比较像距 $v$ 与物距 $u$ 再判断比例。`], ["成像性质", currentAnswerIs(c, "实像") || currentAnswerIs(c, "虚像") ? "先检查光线是否真实到达光屏、像能否被光屏承接，再根据上下特征点光线是否交叉判断正倒；不要用名称直接猜性质。" : "光线真实到达光屏形成实像，上下特征点的光线交叉使像倒立；它不是镜后反向延长线形成的虚像。"], ["变化复核", `${c.question} 只改变一个距离后，重新画两条边界光线并核对相似三角形比例，不能凭“更近看起来更大”直接猜。`]);
}
function physicsLensBlocks(c) {
    ...
    return blocks(["成像分区", `${c.task} 先标出${lens}、光心、两侧${focusTerm}与二倍特征距离位置，再把题给物距放进对应区间。`], ["主光线", `${c.clue} ${rayRule}`], ["交点判断", hideImageType ? `${c.approach} 先判断两条实际折射光线是否真正相交；若不相交，再用反向延长线寻找观察到的位置，并据此判断能否被光屏承接。` : `${c.approach} 两条实际折射光线的交点对应实像；若实际光线不相交，则用反向延长线的交点判断虚像位置。`], ["像的四要素", "先由交点所在侧判断位置和实虚，再由像相对主轴的方向判断正倒，最后比较像高与物高判断放大、等大或缩小。"], ["光屏复核", hideImageType ? `${c.question} 用光屏能否清晰承接来复核成像性质，再用物距区间与两条主光线复画一次，避免直接背名称。` : `${c.question} 实像可在光屏上承接，虚像不能；再用物距区间与两条主光线复画一次，避免把记忆表格套错区间。`]);
}
function physicsOpticsBlocks(c) {
    ...
    if (mirrorContext && mirrorImageTask)
        return physicsMirrorImageBlocks(c);
    if (/(?:平面镜|镜面|镜子|反射角|反射光|返回光线)/.test(opticalText))
        return physicsReflectionBlocks(c);
...
function currentAnswerIs(context, term) {
    return context.answer.normalize("NFKC").trim() === term;
}
function safeGeneratedField(value, answer, fallback) {
    const candidate = value?.trim() ?? "";
    return candidate && !(0, answer_protection_1.generatedTextContainsAnswer)(candidate, answer) ? candidate : fallback;
}
function evidenceClauses(text) {
    const clauses = (0, board_evidence_1.extractBoardEvidenceClauses)(text).filter((item) => item.length >= 4);
...
function profile(discipline, label, thesis, moves) { return { discipline, label, thesis, moves }; }
function move(id, label, role, purpose, selfCheck) { return { id, label, role, purpose, selfCheck }; }
function clean(value, maximum) {
    const text = value.replace(/^#{1,6}\s*/gm, "").replace(/_{2,}/g, (blank) => "＿".repeat(blank.length)).replace(/[*`>#]/g, "").replace(/\s+/g, " ").trim();
    if (text.length <= maximum && isBalanced(text))
        return text;
    for (let end = Math.min(maximum, text.length); end >= Math.min(12, maximum); end -= 1) {
        ...
        if (isBalanced(candidate))
            return end < text.length ? `${candidate}…` : candidate;
    }
    return text.replace(/[$\\]/g, "").slice(0, maximum).trim();
}
function isBalanced(value) { try {
    (0, presentation_1.assertBalancedLearningMarkup)(value, "学科原生板书文本");
    return true;
}
catch {
    return false;
