...
exports.prepareStepAnswerMarkdown = prepareStepAnswerMarkdown;
...
function prepareStepAnswerMarkdown(source) {
    ...
    if (commands.some((command) => !known.test(command.slice(1))))
        return source;
    const operands = value.replace(/\\[A-Za-z]+/g, "");
    // Do not reinterpret prose, code, paths or unsupported commands as math.
    if (!/^[A-Za-z0-9\s{}()[\]^_+\-*/=<>|.,!:;≥≤≠−]*$/.test(operands) || /[A-Za-z]{2,}/.test(operands))
        return source;
    return `$${value}$`;
}
