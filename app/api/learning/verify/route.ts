import { advanceAfterMastery, fail, ok } from "@/lib/learning/api";
import { canVerifyNode, isReadyForOriginal } from "@/lib/learning/graph";
...
import type { AssessmentEvidence } from "@/lib/learning/types";
  ...
  try {
    ...
    if (body.nodeId === "__transfer__") {
      ...
      session = {
        ...
        stage: result.passed ? "complete" : "transfer_check",
        evidence: session.evidence.concat({ nodeId: "__transfer__", source: "system", answer: body.answer, passed: result.passed, createdAt: now }),
        updatedAt: now,
      };
      return ok(session.provider, adapter.modelId, { ...toClientState(session), assessment: result });
    }
    ...
    if (node.kind === "problem") {
      ...
      if (body.source === "parent") throw new Error("原题必须由孩子独立完成，不能由家长代为确认");
    ...
    if (body.source === "parent") {
      if (node.kind === "problem" || node.state === "needs_help") throw new Error("当前节点不能由家长覆盖确认");
      passed = true; source = "parent"; explanation = "已按家长判断记录；最终仍需通过原题和迁移题。";
    } else if (explicitlyUnknown) {
      explanation = node.atomic ? "先换一种讲法和更具体的例子，再检查一次。" : "已记录为不会，可以继续向下拆解。";
    } else {
      if (typeof body.answer !== "string" || !body.answer.trim()) throw new Error("请先提交孩子的答案");
      const result = await adapter.verifyAnswer(node.check, body.answer);
      passed = result.passed; explanation = result.explanation;
    }

    const attempts = node.attempts + (source === "system" ? 1 : 0);
    const objectiveCheckFailed = !passed && !explicitlyUnknown;
    const nextState = passed
      ? (source === "parent" ? "parent_confirmed" : "mastered")
      : node.atomic && objectiveCheckFailed && attempts >= 2
        ? "needs_help"
        : "unknown";
    session = {
      ...session,
      nodes: session.nodes.map((item) => item.id === node.id ? { ...item, attempts, state: nextState } : item),
      evidence: session.evidence.concat({ nodeId, source, answer: source === "system" && !explicitlyUnknown ? body.answer : undefined, passed, createdAt: now }),
      updatedAt: now,
    };
