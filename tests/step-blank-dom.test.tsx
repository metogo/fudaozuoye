...
vi.mock("next/dynamic", () => ({ default: () => ({ onConfirm, onCancel, statusMessage }: ComponentProps<typeof WhiteboardInput>) => <div><button onClick={() => onConfirm(new Blob(["ink"]), "blob:ink")}>提交手写</button><button onClick={onCancel}>取消手写</button><span>{statusMessage}</span></div> }));
...
const gate: ComponentProps<typeof StepBlank>["gate"] = { id: "g1", title: "补全判别式", kind: "step_answer", prompt: "补全判别式", stepBlank: { before: "Δ", after: "≥ 0", hint: "写出判别式" }, stepAnswer: { answer: "b^2-4ac", explanation: "判别式公式" } };
