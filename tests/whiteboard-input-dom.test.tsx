...
describe("WhiteboardInput", () => { afterEach(cleanup);
 ...
 it("导出失败会显示可读错误", async () => { render(<WhiteboardInput taskLabel="写" submitLabel="提交" onConfirm={vi.fn()} onCancel={vi.fn()}/>); fireEvent.click(screen.getByText("落笔")); api.exportImage.mockResolvedValueOnce(""); fireEvent.click(screen.getByRole("button", { name: "提交" })); expect((await screen.findByRole("alert")).textContent).toContain("白板还没有可发送的内容"); });
});
