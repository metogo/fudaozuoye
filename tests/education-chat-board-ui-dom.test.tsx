...
vi.mock("@/components/learning-chat", () => ({
  LearningChat: (props: any) => { chat = props; return <div data-testid="chat"/>; },
}));
vi.mock("@/components/learning-board", () => ({
  LearningBoard: (props: any) => { board = props; return <section data-testid="board"/>; },
}));
...
const session: any = {
  ...
  flow: { stage: "core_explanation", viewedSolution: false, suggestedQuestions: [], activeGate: { id: "gate", kind: "understanding", title: "理解", prompt: "是否理解", options: [{ id: "view_board", label: "看板书" }, { id: "continue", label: "继续" }] } },
};
const lesson: any = {
...
describe("隐藏板书的恢复路径", () => {
  ...
  it("可打开、校验、提问、收起并重新打开一份生成好的板书", async () => {
    ...
    board.onWorkspaceChange({ invalid: true });
    await waitFor(() => expect(chat.notice).toContain("主动回忆状态异常"));
    await act(async () => { await board.onAsk("为什么要除以时间？"); });
    expect(chat.messages.some((message: any) => message.surface === "board")).toBe(true);
