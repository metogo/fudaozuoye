...
let explode = false;
vi.mock("@/components/board-scene-visual", () => ({ BoardSceneVisual: ({ visual }: { visual: BoardSemanticVisual }) => { if (explode) throw new Error("draw failed"); return <div>图示 {visual.title}</div>; } }));
import { BoardVisualBoundary } from "@/components/board-visual-boundary";
const visual: BoardSemanticVisual = { kind: "formula_chain", title: "公式", evidence: "条件", caption: "说明", steps: [{ id: "s", expression: "a=b", explanation: "列式" }] };
