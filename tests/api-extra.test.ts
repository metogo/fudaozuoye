...
describe("会话 API 边界", () => {
  it("兼容旧会话缺失的可选流程字段，并补上待看答案入口", () => {
    ...
    const session = { ...current, reasoningLevel: undefined, flow: { ...current.flow, stage: "original_attempt", activeGate: { id: "g", kind: "original_answer", title: "原题", nodeId: current.rootNodeId }, boardSuggestion: undefined, suggestedQuestions: undefined, solutionRecallPassed: undefined } };
  ...
  it("成功与失败信封保留可读错误和可重试语义", async () => {
    const success: unknown = await ok("doubao", "model", { value: 1 }, "req-fixed").json();
    const retry: unknown = await fail(new ServiceError("忙", 503, "PROVIDER_BUSY", true)).json();
    const invalid: unknown = await fail("bad").json();
    expect(success).toMatchObject({ requestId: "req-fixed", data: { value: 1 }, error: null });
    expect(retry).toHaveProperty("error", expect.objectContaining({ code: "PROVIDER_BUSY", retryable: true }));
    expect(invalid).toHaveProperty("error", expect.objectContaining({ code: "INVALID_REQUEST", retryable: false }));
