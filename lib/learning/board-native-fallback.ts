...
export function inferBoardSubject(session: LearningSession): BoardTeachingSubject {
  ...
  if (session.problem.subject === "physics" || session.problem.subject === "chemistry") return "science";
  if (/(?:语文|英语|阅读|文章|段落|句子|词语|古诗|文言|作文|修辞|主旨|语法|翻译)/.test(content)) return "language";
  if (/(?:历史|政治|道德|法治|朝代|事件|制度|人物|原因|影响|意义|材料题)/.test(content)) return "humanities";
  if (session.problem.subject === "math") return "math";
  if (/(?:物理|化学|生物|地理|速度|力|电路|电流|电压|透镜|光屏|反应|溶液|实验|细胞|生态|气候)/.test(content)) return "science";
  if (/(?:函数|方程|几何|三角形|角|证明|数列|概率|分数|倍数|面积|体积|坐标|代数|不等式|多项式)/.test(content)) return "math";
  return "general";
}
...
export function createNativeBoardFallbackPlan(session: LearningSession, blocks: BoardBlock[]): BoardPlan {
  ...
  const scenes = blocks.map((block, index): BoardScene => ({
    id: block.id,
    intent: (["extract", "connect", "derive", "verify", "compare", "verify"] as BoardScene["intent"][])[index] ?? "verify",
...
function nodeBlocks(node: KnowledgeNode): BoardBlock[] {
...
function problemBlocks(session: LearningSession): BoardBlock[] {
  ...
  const orient: BoardBlock = { id: "board-1", label: "读题压缩", content: `先把任务压成一句话：${goal} 再把暂时不参与第一步的信息放到旁边。`, tone: "plain" };
  const recap: BoardBlock = { id: "board-6", label: "一页记忆", content: `最后只记三件事：题目要什么、哪条条件决定核心关系、第一步为什么成立。自查问题是：${firstQuestion}`, tone: "key" };
...
function mathRelationModel(problem: string): string {
  if (/(?:函数|方程|=)/.test(problem)) {
    ...
    if (relation && assigned) return `题干明确给出关系式 $${relation}$ 和指定值 $${assigned}$。先确认指定值对应哪个字母，再把它放回关系式的正确位置；这里只建立代入关系，不提前计算最终结果。`;
    if (relation) return `先把题干中的关系式 $${relation}$ 作为固定结构，再区分已知量和待求量；每次变形都说明使用了哪一项关系，不提前计算最终结果。`;
    return "把表达式、给定值和待求量放进同一结构：先确认给定值对应哪个字母，再把它放回关系式的正确位置；这里只建立代入或变形关系，不提前计算最终结果。";
  }
  if (/(?:三角形|几何|证明|角|边|圆)/.test(problem)) return "把点、边、角和已知性质放回同一图形，先找能连接已知与目标的中间关系；每条关系都要能指出对应条件，不能只凭图形外观判断。";
  return "把已知量、待求量和限制条件放进同一数量关系，先确认谁随谁变化、单位是否一致，再选择能从已知走向目标的表达方式。";
}

function extractMathRelation(problem: string): string | null {
...
function cleanCue(value: string, maximum: number): string {
  const plain = value
    .replace(/^#{1,6}\s*/gm, "")
    .replace(/^\s*(?:[-*+]\s+|\d+[.)、]\s*)/gm, "")
    .replace(/[*_`>#]/g, "")
    .replace(/\s+/g, " ")
    .trim();
  if (plain.length <= maximum) return balancedOrPlain(plain);
...
function safeBoardThesis(session: LearningSession): string {
  const subject = inferBoardSubject(session);
  if (subject === "math") return "把题干条件翻译成可检查的数学关系，再沿着依据推进关键步骤。";
  if (subject === "science") return "先确定研究对象和过程，再用条件解释变量、现象与结论之间的联系。";
  if (subject === "language") return "从原文证据出发，先看结构和语境，再解释表达为什么成立。";
  if (subject === "humanities") return "把材料、背景与问题要求连成证据链，再区分原因、过程和影响。";
...
function boardEvidenceCandidates(session: LearningSession): string[] {
  ...
  ].filter((value): value is string => typeof value === "string").map((value) => value.trim().replace(/\s+/g, " ")).filter((value) => value.length >= 4);
...
function safeLearningSlice(value: string, maximum: number): string {
  ...
  for (let end = maximum; end >= Math.min(12, maximum); end -= 1) {
    ...
    if (isBalancedLearningMarkup(candidate)) return candidate;
  }
  return value.replace(/[$`\\]/g, "").slice(0, maximum).trim();
}

function balancedOrPlain(value: string): string {
  return isBalancedLearningMarkup(value) ? value : value.replace(/[$`\\]/g, "");
}

function isBalancedLearningMarkup(value: string): boolean {
  try {
    assertBalancedLearningMarkup(value, "安全板书文本");
    return true;
  } catch {
    return false;
  }
...
function teachingRole(index: number, count: number): BoardTeachingRole {
  if (index === 0) return "orient";
  if (index === 1) return "model";
  if (index === 2) return "reason";
  if (index === count - 1) return "recap";
  return count >= 6 && index === count - 2 ? "transfer" : "misconception";
}

function teachingPurpose(index: number, count: number): string {
  return {
...
function teachingWhy(index: number, count: number): string {
  return {
...
function teachingCheck(index: number, count: number): string {
  return {
