...
export function knowledgeGuide(messages: ChatMessage[]): string | null {
  const lesson = messages.find(message => message.role === "assistant" && message.kind === "assistant" && message.surface !== "board");
  if (!lesson || lesson.status !== "complete" || lesson.scopeLabel === "原题完整讲解") return null;
  const quotes = (lesson.emphasis ?? []).filter(mark => {
    if (!mark || typeof mark.target !== "string") return false;
