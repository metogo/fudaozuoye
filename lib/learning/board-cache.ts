...
import { createNativeBoardBlocks, createNativeBoardFallbackPlan } from "./board-native-fallback";
import { parseBoardPlan } from "./providers/board-plan";
...
export function restoreBoardLesson(session: LearningSession, value: unknown): BoardLesson | null {
  ...
  if (!nativeLesson.plan) return null;
  let verifiedPlan;
  try {
    ...
    verifiedPlan = parseBoardPlan(planForValidation, session, nativeLesson.blocks, []);
  } catch (error) {
    console.warn("板书缓存未通过事实复检", error instanceof Error ? error.message : "未知错误");
    return null;
  }
  const lesson = enrichBoardLessonWithSafeAids(session, { ...nativeLesson, plan: verifiedPlan });
...
function isStoredBoardPlan(value: unknown, blocks: BoardLesson["blocks"]): boolean {
  ...
  if (plan.contentRevision !== undefined && plan.contentRevision !== 1) return false;
...
function upgradeLegacyBoardLesson(session: LearningSession, legacy: BoardLesson): BoardLesson {
  const focus = session.flow.focus;
  const blocks = createNativeBoardBlocks(session, focus);
  return {
    ...legacy,
