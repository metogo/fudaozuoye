import type { BoardConversationMessage, CheckItem, LearningGate, LearningSession } from "./types";
import { assertBalancedLearningMarkup } from "./presentation";

export interface StepExercise { check: CheckItem; blank: NonNullable<LearningGate["stepBlank"]> }

/** IDs are scoped to this exact displayed explanation, never supplied by the model. */
export function stepSourceSegments(source: string): { id: string; text: string }[] {
  return source.split(/\n\s*\n/).map((text) => text.trim()).filter(Boolean)
    .map((text, index) => ({ id: `step-source-${index + 1}`, text }));
}

export function stepSource(session: LearningSession, context: BoardConversationMessage[]): string {
...
export function parseStepExercise(value: Record<string, unknown>, source: string): StepExercise {
  const field = (key: string, max: number, optional = false) => {
    const raw = value[key];
    if (typeof raw !== "string" || (!optional && !raw.trim()) || raw.length > max) throw new Error(`当前步骤的 ${key} 不完整`);
    assertBalancedLearningMarkup(raw, "步骤填空");
    return raw.trim();
  };
  if (value.sourceId !== undefined) {
    const segment = stepSourceSegments(source).find((item) => item.id === value.sourceId);
    if (!segment || segment.text.length < 4) throw new Error("填空引用的讲解片段编号无效");
  } else {
    // Backward compatibility for adapters still returning a literal quote.
    const quote = field("sourceQuote", 350);
    if (quote.length < 4 || !source.includes(quote)) throw new Error("填空未对应刚才讲解的步骤");
  }
  ...
  if (/重做整题|完成整题|分别求出|所有问题/.test(instruction) || /_{2,}|\[blank\]|□/.test(before + after)) throw new Error("一次只能练一个关键空");
