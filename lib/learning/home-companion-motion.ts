...
export function animateHomeCompanion(root: HTMLElement) {
  ...
  const play = (intro = false) => {
    ...
    } catch { settle(); } // Unsupported animations must leave the whole name readable.
  };
  const pause = () => { clearTimeout(timer); settle(); };
  const onVisibility = () => { pause(); schedule(); };
  const onFocus = () => { if (typing()) pause(); };
  ...
  const onClick = (event: Event) => {
    if ((event.target as Element).closest("[data-companion-replay]")) play(false);
  };
