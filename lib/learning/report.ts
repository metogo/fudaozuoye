import type { LearningSession, NodeState } from "./types";

const subjects = { math: "数学", physics: "物理", chemistry: "化学", biology: "生物", chinese: "语文", english: "英语", history: "历史", geography: "地理", politics: "政治" } as const;
...
export async function createReportFile(session: LearningSession): Promise<File> {
  ...
  if (!context) throw new Error("当前浏览器无法生成学习报告");
  ...
  concepts.forEach((node, index) => {
    ...
    context.beginPath(); context.arc(116, y + 48, 16, 0, Math.PI * 2); context.fill();
...
function wrapText(context: CanvasRenderingContext2D, text: string, x: number, y: number, maxWidth: number, lineHeight: number) {
  ...
  for (const char of text) {
    ...
    if (context.measureText(candidate).width > maxWidth) { context.fillText(line, x, y); line = char; y += lineHeight; } else line = candidate;
  }
