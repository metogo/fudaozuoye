import type { CheckItem, GradeBand, ProblemSnapshot, Subject } from "./types";

type CheckContent = Pick<CheckItem, "prompt" | "answer" | "explanation">;
type Rubric = { required: RegExp[]; forbidden?: RegExp[] };

const answers: Record<Subject, Record<GradeBand, Omit<CheckContent, "prompt">>> = {
  math: {
...
export function originalCheckContent(problem: ProblemSnapshot): CheckContent {
  return {
...
export function solutionForSample(problem: ProblemSnapshot): string {
...
const solutionSteps: Record<Subject, Record<GradeBand, string[]>> = {
  math: {
...
export function openAnswerRubric(check: CheckItem): Rubric | null {
  const key = `${check.conceptId ?? ""} ${check.prompt}`;
  if (/homeostasis|血糖/.test(key)) return { required: [/(?:胰岛素|insulin)/i, /(?:降低|下降|摄取|糖原)/, /(?:负反馈|稳态|恢复)/], forbidden: [/(?:不需要|没有|无)反馈|胰岛素.*(?:升高|增加)血糖/] };
  if (/biology|探究|对照实验/.test(key)) return { required: [/(?:光照|温度|研究因素|自变量)/, /(?:对照|两组|实验组)/, /(?:(?:其他条件|控制变量|水分|温度).*(?:相同|一致|保持))|(?:(?:相同|一致|保持).*(?:其他条件|控制变量|水分|温度))/], forbidden: [/(?:变量不重要|不要设置对照|无需对照|未保持一致|没有保持一致|其他条件(?:全部)?不同|其他条件不用保持|随机设置)/] };
  if (/chinese|阅读|赏析|结尾段/.test(key)) return { required: [/(?:原文|‘|“|拿起|放下|翻过|小猫|真正的勇敢)/, /(?:作用|表现|生动|画面|节奏|情感|主旨|照应|犹豫|机灵)/], forbidden: [/(?:证据|原文).*(?:无关|没用|没有作用)|(?:完全无关|没有任何作用)/] };
  if (/english|Read|Although|passage/i.test(key)) return { required: [/(?:seven|rainbecameheavier|takethebus|istayed|although|textevidence)/i, /(?:shows|change|changed|concession|continued|getsup|decision|because)/i], forbidden: [/(?:although).*(?:not|isn't).*(?:concession)|(?:evidence|text).*(?:irrelevant|unrelated)|matchstopped/i] };
  if (/history|改革|史料/.test(key)) return { required: [/(?:材料|史料|1898|1900|改革|赋税|学堂|制度)/, /(?:影响|变化|背景|原因|过程|措施|比较)/], forbidden: [/(?:没有|不存在|无)(?:背景|影响|变化)|(?:背景|影响|变化).*(?:没有|无关)/] };
  if (/geography|区域|地形|气候|降水/.test(key)) return { required: [/(?:区域|位置|地形|气候|风向|迎风坡|湿润海风)/, /(?:过程|影响|形成|降水|联系|差异|抬升)/], forbidden: [/(?:地形|气候|风向).*(?:没有联系|无关|不会)|(?:不会形成|没有)(?:降水|差异)/] };
  if (/politics|权利|义务|无障碍|传统与创新/.test(key)) return { required: [/(?:材料|权利|义务|无障碍|主体|行为|传统|创新)/, /(?:观点|概念|意义|体现|保护|保障|依法|法治|矛盾)/], forbidden: [/(?:不体现|没有|无)(?:任何)?(?:观点|意义|权利)|(?:权利|义务).*(?:无关|没有)/] };
  return null;
}
