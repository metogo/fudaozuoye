...
const record = (v: unknown): Record<string, unknown> => {
  if (!v || typeof v !== "object" || Array.isArray(v)) throw new Error("知识图谱结构不完整");
...
export function parseKnowledgeMap(value: unknown, evidenceSource: string): ProblemKnowledgeMap {
  ...
  if (ids.size !== nodes.length || new Set(nodes.map(n => n.title)).size !== nodes.length) throw new Error("知识点重复");
  const rootId = clean(raw.rootId, 40);
  if (!ids.has(rootId)) throw new Error("缺少核心知识点");
  const edges: MapRelation[] = raw.edges.map(v => {
    const e = record(v);
    const from = clean(e.from, 40), to = clean(e.to, 40);
    if (!ids.has(from) || !ids.has(to) || from === to || to === rootId || !["prerequisite", "application"].includes(String(e.kind))) throw new Error("知识关系不合法");
    return { from, to, kind: e.kind as MapRelation["kind"], reason: clean(e.reason, 180) };
  });
  if (new Set(edges.map(e => `${e.from}:${e.to}`)).size !== edges.length) throw new Error("知识关系重复");
  const visited = new Set<string>();
  function visit(id: string, path: Set<string>) {
    if (path.has(id)) throw new Error("知识关系出现循环");
    if (path.size > 4) throw new Error("知识图谱层级过深");
    visited.add(id);
    for (const e of edges.filter(e => e.from === id)) visit(e.to, new Set([...path, id]));
  }
  visit(rootId, new Set());
  if (visited.size !== nodes.length) throw new Error("存在未关联的知识点");
  for (const id of [rootId, ...edges.filter(e => e.from === rootId).map(e => e.to)]) {
    if (!nodes.find(n => n.id === id)?.evidence) throw new Error("本题核心知识缺少题目依据");
  }
  return { version: 1, ...(raw.overviewOnly === true ? { overviewOnly: true } : {}), rootId, nodes, edges };
}

export function mapEvidence(session: LearningSession) {
...
export function visibleConceptIds(map: ProblemKnowledgeMap, expanded: Set<string>) {
  const visible = new Set([map.rootId]);
  function visit(id: string) {
    if (!expanded.has(id)) return;
    for (const edge of map.edges.filter(e => e.from === id)) {
      if (!visible.has(edge.to)) { visible.add(edge.to); visit(edge.to); }
    }
  }
  visit(map.rootId);
  return visible;
...
export function arrangeConcepts(map: ProblemKnowledgeMap): Record<string, MapPoint> {
  const ranks = new Map([[map.rootId, 0]]);
  for (let i = 0; i < map.nodes.length; i++) for (const e of map.edges) {
    const rank = ranks.get(e.from);
    if (rank !== undefined) ranks.set(e.to, Math.max(ranks.get(e.to) ?? 0, rank + 1));
  }
...
export function parseMapPositions(value: unknown, map: ProblemKnowledgeMap): Record<string, MapPoint> {
  const result = arrangeConcepts(map);
  if (!value || typeof value !== "object") return result;
  for (const n of map.nodes) {
    const p = (value as Record<string, MapPoint>)[n.id];
    if (p && Number.isFinite(p.x) && Number.isFinite(p.y) && Math.abs(p.x) < 100000 && Math.abs(p.y) < 100000) result[n.id] = { x: p.x, y: p.y };
  }
  return result;
}
