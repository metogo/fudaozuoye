import { subjectBoardProfile, subjectBoardProfileFor, type SubjectBoardProfile } from "./board-subject-engine";
import { isUsefulBoardVisual } from "./board-visual-runtime";
import { problemEvidenceText } from "./problem-evidence";
import type { BoardConversationMessage, BoardScene, LearningSession, TutorScope } from "./types";
 ...
 */
export function directBoardScenes(input: BoardScene[]): BoardScene[] {
  ...
  if (reasoningIndex >= 0) selected.add(reasoningIndex);
  ...
  if (selected.size < MIN_SCENES) selected.add(Math.min(1, scenes.length - 1));
  return scenes.filter((_, index) => selected.has(index));
}

export type DirectedBoardMove = SubjectBoardProfile["moves"][number];

/** 在调用模型前决定本题需要哪些教学动作，避免先生成固定五段再做表面裁剪。 */
export function directBoardBlueprint(session: LearningSession, scope: TutorScope, dialogue: BoardConversationMessage[] = []): DirectedBoardMove[] {
  const profile = subjectBoardProfileFor(session);
  const defaultMoveIds = subjectBoardProfile(session.problem.subject).moves.map((move) => move.id).join("|");
  ...
  const text = `${problemEvidenceText(session.problem)}\n${session.problemGuide.goal}\n${focusText}\n${dialogue.map((item) => item.text).join("\n")}`;
  const indexes = new Set([0, 2]);
  if (/[=<>∠△图表曲线关系结构过程证据原文材料史料公元\d{3,4}年实验变量反应光路语法句式]|word|phrase|sentence|grammar|evidence/i.test(text)) indexes.add(1);
  if (specialized) indexes.add(1);
  const dialogueText = dialogue.map((item) => item.text).join("\n");
  if (/(?:对象|已知|条件|题意|求什么|问什么|关系|联系|对应|结构)/.test(`${focusText}\n${dialogueText}`)) indexes.add(1);
  if (specialized || /(?:推导|变形|计算|结论|不懂|不会|出错|易错|边界|为什么|但是|可是|混淆|区别|辨析|第.{0,3}步|下一步|从哪|怎么来|依据)/.test(dialogueText)) indexes.add(3);
  if (/(?:推导|变形|计算|结论|错误|易错|为什么|依据|反例|限制|混淆|区别)/.test(focusText)) indexes.add(3);
  if (session.problem.subject === "math" && /(?:sin|cos|tan|三角形|证明|定义域|根|概率|数列)/i.test(text)) indexes.add(3);
  if (/(?:迁移|总结|复述|举一反三|检验|验证|完整方法|同类题)/.test(text)) indexes.add(profile.moves.length - 1);
  return profile.moves.filter((_, index) => indexes.has(index)).slice(0, MAX_SCENES);
}

export function assertDirectedBoardMoves(session: LearningSession, scope: TutorScope, dialogue: BoardConversationMessage[], moves: string[]): void {
...
function rankedIndexes(scenes: BoardScene[]): number[] {
...
function sceneScore(scene: BoardScene, index: number): number {
  return Number(isUsefulBoardVisual(scene.visual)) * 12
