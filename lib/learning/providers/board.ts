...
export function createSafeBoardLesson(session: LearningSession, scope: TutorScope, suggestion: BoardSuggestion): BoardLesson {
  const node = scope.kind === "node" ? session.nodes.find((item) => item.id === scope.nodeId && item.kind === "concept") : undefined;
  const blocks: BoardBlock[] = node ? [
...
function createNeutralBoardLesson(session: LearningSession, suggestion: BoardSuggestion): BoardLesson {
...
export function createSafeBoardVisual(session: LearningSession, suggestion: BoardSuggestion): BoardVisual | null {
  ...
  const hasStructuredRelation = suggestion.layout !== "steps"
    || problem.length >= 12 && (
      /(?:已知|若|当|其中|分别|关系|变化|速度|路程|时间|质量|浓度|方程|函数|电路|受力|反应|等于|相比|每|倍|分之)/.test(problem)
      || (problem.match(/[=＋+－\-×÷*/]/g)?.length ?? 0) >= 1
    );
  if (!hasGeometry && !hasOptics && !hasStructuredRelation) return null;

  const evidence = exactEvidence(problem);
  const relationElements: BoardVisualElement[] = [
  ...
  const visual: BoardVisual = hasGeometry
    ? {
        kind: "geometry",
        title: "把图形位置先摆清楚",
        evidence,
...
export function parseBoardAudit(value: JsonObject): { passed: boolean; reason: string } {
  const fields = ["correct", "grounded", "noAnswerLeak", "markingRelevant", "visualCorrect", "visualGrounded"] as const;
