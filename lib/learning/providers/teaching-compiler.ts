import { fraction, unit, type Fraction } from "mathjs";
import type { IllustrationLesson, LearningSession } from "../types";
import { illustrationFingerprint } from "../illustration-fingerprint";
import { teachingSceneSvg, type TeachingScene, type TeachingShape } from "../teaching-scene";

const unsupported = "这道题暂未匹配到可核验的分步演示，请先使用完整讲解。";
const num = "([0-9]+(?:\\.[0-9]+)?)";
const lengthUnit = "(千米|厘米|分米|毫米|米)";
const units: Record<string, string> = { 米: "m", 厘米: "cm", 分米: "dm", 毫米: "mm", 千米: "km" };
type Stage = { id: string; title: string; explanation: string; shapes: TeachingShape[] };
export interface TeachingProgram {
  template: TeachingScene["template"];
  worldId: string;
  sourceQuotes: string[];
  stages: Stage[];
  results: Array<{ value: string; unit: string }>;
}

const f = (value: string | number) => fraction(String(value));
const show = (value: Fraction) => value.d === 1n ? value.n.toString() : value.toFraction();
const n = (value: Fraction) => value.valueOf();
const label = (id: string, x: number, y: number, text: string): TeachingShape => ({ kind: "label", id, x, y, text });
const rect = (id: string, x: number, y: number, width: number, height: number, color: "base" | "change" | "outline" = "base"): TeachingShape => ({ kind: "rect", id, x, y, width, height, color });

/** Restrictive recognizers establish semantic roles before any model planning occurs. */
export function compileTeachingProgram(session: LearningSession): TeachingProgram {
  if (session.problem.subject !== "math") throw new Error(unsupported);
  if (session.problem.visualContext?.affectsSolving) throw new Error("这道题依赖原图条件，暂不提供自动分步演示，请使用完整讲解。");
  const text = session.problem.text.replace(/\s/g, "");
  if (text.length > 1200 || /至少|至多|大约|近似|阴影|重叠|切去|挖去|剩余|折扣|百分|%|速度.*改变|加速|[-−]\d/.test(text)) throw new Error(unsupported);
  const root = session.nodes.find((node) => node.id === session.rootNodeId);
  if (!root?.check.answer.trim() || !root.check.explanation.trim()) throw new Error("原题解答尚未核验，暂时不能生成演示。");
  ...
  if (!program) throw new Error(unsupported);
  ...
  for (const result of program.results) {
    ...
    if (!found) throw new Error("演示计算与当前解答尚未核对一致，请先查看完整讲解。");
  }
  ...
  if (answerValues.some((match) => !program.results.some((result) => result.unit === match[2] && f(result.value).equals(f(match[1]))))) throw new Error("演示尚未覆盖这道题的全部结论，请使用完整讲解。");
  for (const stage of program.stages) {
    if (!stage.shapes.length || stage.shapes.length > 100) throw new Error(unsupported);
    for (const shape of stage.shapes) {
      for (const value of Object.values(shape)) if (typeof value === "number" && (!Number.isFinite(value) || Math.abs(value) > 2000)) throw new Error(unsupported);
    }
  }
  return program;
}

function program(template: TeachingProgram["template"], text: string, stages: Stage[], results: TeachingProgram["results"]): TeachingProgram {
  return { template, worldId: `world-${template}`, sourceQuotes: [text], stages, results };
}

function hasOnlyQuantities(text: string, expected: number): boolean {
...
function rectangleProgram(text: string): TeachingProgram | null {
  if (!/长方形|矩形/.test(text)) return null;
  ...
  if (!hasOnlyQuantities(text, change ? 3 : 2)) return null;
  if (/增加|延长|减少|缩短/.test(text) && (!change || !/宽(?:保持)?不变/.test(text))) return null;
  if (/宽(?:增加|延长|减少)|周长不变|面积不变|正方形/.test(text)) return null;
  const u = l[2];
  const L = f(l[1]);
  const W = f(w[1]).mul(f(String(unit(1, units[w[2]]).toNumber(units[u]))));
  const D = change ? f(change[1]).mul(f(String(unit(1, units[change[2]]).toNumber(units[u])))) : f(0);
  if (n(L) <= 0 || n(W) <= 0 || n(D) < 0 || n(L.add(D)) > 1e6) return null;
  ...
  if (!wantsPerimeter && !wantsArea) return null;
  if (change && /原(?:来|菜园|长方形)(?:的)?周长/.test(text)) return null;
  const scale = Math.min(510 / n(next), 220 / n(W));
  const x = 130, y = 170, width = n(L) * scale, height = n(W) * scale, dw = n(D) * scale;
  if (Math.min(width, height) < 30 || (change && dw < 12)) return null;
  ...
  const stages: Stage[] = [{ id: "given", title: "先看原来的长和宽", explanation: `原长${show(L)}${u}，宽${show(W)}${u}。横边表示长，竖边表示宽。`, shapes: original }];
  if (change) stages.push({ id: "extend", title: "只延长长边，宽保持不变", explanation: `新长：${show(L)} + ${show(D)} = ${show(next)}（${u}）。黄色部分与原图等高，表示宽不变。`, shapes: [...extended, label("new-length", x + (width + dw) / 2, y + height + 38, `新长 ${show(next)}${u}`)] });
  const results: TeachingProgram["results"] = [];
  if (wantsPerimeter) {
...
function bars(count: number, each: Fraction, u: string, reveal: boolean): TeachingShape[] {
...
function groupsProgram(text: string): TeachingProgram | null {
  if (!hasOnlyQuantities(text, 2)) return null;
  const each = text.match(new RegExp(`每(?:盒|袋|组|份|箱)(?:有|装有|装)?${num}(个|本|支|颗|张|块)`));
  if (!each || !/一共|共有|总共|总数/.test(text)) return null;
  const container = each[0][1];
  const countMatch = text.match(new RegExp(`${num}${container}`));
  if (!countMatch || /又|剩|卖|取出|其中|每.*每/.test(text)) return null;
  const count = Number(countMatch[1]), E = f(each[1]), u = each[2];
  if (!Number.isInteger(count) || count < 2 || count > 10 || n(E) <= 0 || !Number.isInteger(n(E)) || (show(E).length + 1) * 19 > 540 / count - 8) return null;
...
function sharingProgram(text: string): TeachingProgram | null {
  if (!hasOnlyQuantities(text, 2)) return null;
  const totalMatch = text.match(new RegExp(`${num}(个|本|支|颗|张|块)`));
  const countMatch = text.match(new RegExp(`平均分(?:给|成)${num}(人|份|组)`));
  if (!totalMatch || !countMatch || !/每.*(?:多少|几)/.test(text) || /剩|余|又|还/.test(text)) return null;
  const total = f(totalMatch[1]), count = Number(countMatch[1]), u = totalMatch[2];
  if (!Number.isInteger(count) || count < 2 || count > 10 || n(total) <= 0 || !Number.isInteger(n(total.div(count)))) return null;
  const E = total.div(count);
  if ((show(E).length + 1) * 19 > 540 / count - 8) return null;
...
function rateProgram(text: string): TeachingProgram | null {
  if (!hasOnlyQuantities(text, 3)) return null;
  const known = text.match(new RegExp(`${num}(小时|分钟)(?:行驶|走|行|跑)(?:了)?${num}(千米|米)`));
  const target = [...text.matchAll(new RegExp(`${num}(小时|分钟)`, "g"))].at(-1);
  if (!known || !target || !/照这样|同样.*速度|速度不变|匀速/.test(text) || !/(?:多少|几)(?:千米|米)/.test(text) || /相遇|追及|返回|休息|同时/.test(text)) return null;
  if (known[2] !== target[2]) return null;
  const T = f(known[1]), D = f(known[3]), Q = f(target[1]), u = known[4], tu = known[2];
  if (n(T) < 1 || n(D) <= 0 || n(Q) <= 0 || T.equals(Q)) return null;
  const speed = D.div(T), result = speed.mul(Q), max = Math.max(n(D), n(result)), scale = 510 / max;
  if (Math.min(n(D), n(result)) * scale < 35) return null;
...
function comparisonProgram(text: string): TeachingProgram | null {
  if (!hasOnlyQuantities(text, 2)) return null;
  const quantities = [...text.matchAll(new RegExp(`${num}(个|本|支|颗|张|元|块)`, "g"))];
  if (quantities.length !== 2 || quantities[0][2] !== quantities[1][2] || !/多多少|少多少|相差多少|一共.*多少|共有多少/.test(text) || /倍|每|又|给了|卖|增加|减少|剩|比.{0,12}(?:多|少)\d/.test(text)) return null;
  ...
  if (Math.min(n(A), n(B)) * scale < 35) return null;
...
function fractionProgram(text: string): TeachingProgram | null {
  if (!hasOnlyQuantities(text, 3) || !/多少|几/.test(text)) return null;
  const totalMatch = text.match(new RegExp(`${num}(个|本|支|颗|张|块)`));
  const partMatch = text.match(/(?:其中的|它的|总数的|的)([1-9]\d*)\/([1-9]\d*)/);
  if (!totalMatch || !partMatch || /剩|余|再|又/.test(text)) return null;
  const count = Number(partMatch[2]), selected = Number(partMatch[1]), total = f(totalMatch[1]), u = totalMatch[2];
  if (count > 10 || selected >= count || n(total) <= 0 || !Number.isInteger(n(total.div(count)))) return null;
  const each = total.div(count), result = each.mul(selected), width = 540 / count;
  if ((show(each).length + 1) * 19 > width - 8) return null;
  const blocks = (highlight: boolean): TeachingShape[] => Array.from({ length: count }, (_, i) => [rect(`part-${i}`, 130 + i * width, 210, width, 100, highlight && i < selected ? "change" : "base"), label(`part-label-${i}`, 130 + (i + .5) * width, 270, `${show(each)}${u}`)]).flat();
...
export function teachingPlannerPrompt(program: TeachingProgram) {
  return {
...
export function assembleTeachingLesson(session: LearningSession, program: TeachingProgram, plan?: unknown): IllustrationLesson {
  let groups = program.stages.map((stage) => [stage.id]);
  if (plan !== undefined) {
    const candidate = (plan as { groups?: unknown })?.groups;
    if (!Array.isArray(candidate) || candidate.length < 2 || candidate.length > 6 || candidate.some((g) => !Array.isArray(g) || !g.length || g.length > 2 || g.some((id) => typeof id !== "string")) || JSON.stringify(candidate.flat()) !== JSON.stringify(program.stages.map((s) => s.id))) throw new Error("步骤编排没有完整覆盖已核验过程");
    groups = candidate as string[][];
    if (groups.some((group, index) => group.length > 1 && (index !== 0 || group[0] !== "given"))) throw new Error("独立的演算变化必须保留对应画面");
  }
  const frames = groups.map((group, index) => {
    const stages = group.map((id) => program.stages.find((stage) => stage.id === id)!);
    const last = stages.at(-1)!;
    const scene: TeachingScene = { version: 1, template: program.template, worldId: program.worldId, stageIds: group, shapes: last.shapes, sourceQuotes: program.sourceQuotes };
    return { id: `frame-${index + 1}`, index: index + 1, title: stages.map((s) => s.title).join("；"), calculation: stages.map((s) => s.explanation).join("\n\n"), transition: index ? `沿用同一题目的数量与比例，继续${last.title}。` : "从原题给出的条件开始。", alt: stages.map((s) => s.explanation).join(" "), imageUrl: `data:image/svg+xml;base64,${Buffer.from(teachingSceneSvg(scene)).toString("base64")}`, scene };
  });
  return { version: 1, requestId: session.requestId, problemFingerprint: illustrationFingerprint(session), title: "跟着图形，一步一步理解", frameCount: frames.length, frames };
}
