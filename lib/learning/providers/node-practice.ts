import { parseNodePractice, type NodePractice } from "../node-practice";
...
type Request = (system: string, prompt: string, image?: string, json?: boolean, timeout?: number, signal?: AbortSignal, tokens?: number) => Promise<string>;

export async function generateNodePractice(session: LearningSession, concept: MapConcept, previous: string[], request: Request): Promise<NodePractice> {
  ...
  const context = { subject: session.problem.subject, grade: session.problem.learnerBand ?? session.problem.gradeBand,
    original: problemEvidenceText(session.problem), concept: { title: concept.title, evidence: concept.evidence }, previous };
...
只输出 JSON：{"question":"题干","options":["选项内容","选项内容","选项内容"],"correctIndex":0,"explanation":"依据和推理","connection":"与原题的联系"}。公式用完整 $LaTeX$，公式内的中文名称必须放在 \\text{...} 中。`, JSON.stringify(context), value => value.available === false ? null : validatedPractice(value), 1700);
  if (!candidate) throw new Error("当前知识点暂不能可靠生成练习，请继续原题讲解");
  if (previous.some(question => question.replace(/\s/g, "") === candidate.question.replace(/\s/g, ""))) throw new Error("新练习与上一题重复，请重试换题");
...
只输出 JSON：{"approved":true,"correctIndex":0,"explanation":"独立推理过程"}。解释只给出正确答案的必要依据和逐步计算，不猜测错误选项由什么错误计算得到。公式使用完整 $LaTeX$。`,
    JSON.stringify({ ...context, previous: undefined, question: candidate.question, options: candidate.options, connection: candidate.connection }), value => {
      if (typeof value.approved !== "boolean") throw new Error("复核结果缺少明确判断");
      if (value.approved && value.correctIndex === candidate.correctIndex) validatedPractice({ ...candidate, explanation: value.explanation });
      return value;
    }, 1400);
  if (review.approved !== true || review.correctIndex !== candidate.correctIndex) throw new Error("练习未通过一致性检查，请重试或继续原题讲解");
  return validatedPractice({ ...candidate, explanation: review.explanation });
}

function validatedPractice(value: unknown) {
  const result = parseNodePractice(value);
  for (const text of [result.question, ...result.options, result.explanation, result.connection]) assertValidLatex(prepareLearningMarkdown(text), "练习");
  return result;
}
...
function boundedPracticeReader(request: Request) {
  const deadline = Date.now() + 40000;
  let corrected = false;
  return async <T>(system: string, prompt: string, validate: (value: Record<string, unknown>) => T, tokens: number): Promise<T> => {
    let input = prompt;
    for (;;) {
      const remaining = deadline - Date.now();
      if (remaining <= 0) throw new DOMException("练习生成超时，请重试", "TimeoutError");
