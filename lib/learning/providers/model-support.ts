...
export function responsesBody(model: string, system: string, prompt: string, image?: string, maxOutputTokens?: number | null) {
...
export function chatBody(model: string, system: string, prompt: string, image?: string, jsonMode = false, disableThinking = false, maxTokens: number | null = 3000) {
...
export function transferCheckTool(): JsonObject {
  return {
    type: "function",
    function: {
...
export function problemSolutionTool(): JsonObject {
  return {
    type: "function",
    function: {
...
export function boardSuggestionTool(): JsonObject {
  return {
    type: "function",
    function: {
...
export function similarCheckTool(type: CheckItem["type"]): JsonObject {
  return {
    type: "function",
    function: {
...
export function parseSimilarCheck(value: JsonObject, target: KnowledgeNode): CheckItem {
  const prompt = typeof value.prompt === "string" ? value.prompt.trim() : "";
  const answer = typeof value.answer === "string" ? value.answer.trim() : "";
  const explanation = typeof value.explanation === "string" ? value.explanation.trim() : "";
  if (prompt.length < 6 || answer.length < 1 || explanation.length < 4) throw new Error("相似题缺少题干、答案或解题依据");
  if (compactCheckText(prompt) === compactCheckText(target.check.prompt)) throw new Error("相似题不能复述当前检查题");
  if (value.type !== target.check.type) throw new Error("相似题的作答形式与当前检查不一致");
  const choices = Array.isArray(value.choices) ? value.choices.filter((choice): choice is string => typeof choice === "string").map((choice) => choice.trim()).filter(Boolean) : [];
  if (target.check.type === "choice") {
    if (choices.length < 2 || choices.length > 5 || new Set(choices.map(compactCheckText)).size !== choices.length) throw new Error("相似选择题的选项不合法");
    if (!choices.some((choice) => compactCheckText(choice) === compactCheckText(answer))) throw new Error("相似选择题的标准答案不在选项中");
  }
...
export function extractToolArguments(payload: JsonObject): string {
...
export function extractText(payload: JsonObject, protocol: ProviderConfig["protocol"]): string {
...
export function emitProviderDelta(line: string, protocol: ProviderConfig["protocol"], onDelta: (text: string) => void): boolean {
  ...
  if (protocol === "chat-completions") {
    const choices = event.choices as Array<{ delta?: { content?: string }; finish_reason?: string | null }> | undefined;
    const choice = choices?.[0];
    const delta = choice?.delta?.content;
    if (delta) onDelta(delta);
    if (choice?.finish_reason === "length") throw new ServiceError("完整讲解因输出长度限制被截断，请重试当前操作", 502, "PROVIDER_OUTPUT_LIMIT", true);
    if (choice?.finish_reason && choice.finish_reason !== "stop") throw new Error("模型没有完整结束本次讲解，请重试当前操作");
  ...
  if (event.type === "response.incomplete") {
    ...
    if (response?.incomplete_details?.reason === "max_output_tokens") throw new ServiceError("完整讲解因输出长度限制被截断，请重试当前操作", 502, "PROVIDER_OUTPUT_LIMIT", true);
    throw new ServiceError("模型没有完整结束本次讲解，请重试当前操作", 502, "PROVIDER_INCOMPLETE", true);
  }
...
export function selectionOutputExample(includeOriginal: boolean): JsonObject {
  return {
...
export function teachingOutputExample(): JsonObject {
  return {
...
function compactCheckText(value: string): string {
