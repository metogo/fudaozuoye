import { directBoardBlueprint } from "../board-director";
import { nativeMoveWhy } from "../board-native-fallback";
import { subjectBoardProfileFor } from "../board-subject-engine";
import { gradeTeachingInstruction } from "../grade-pedagogy";
import { problemEvidenceText } from "../problem-evidence";
import type { BoardConversationMessage, BoardSuggestion, GradeBand, LearningSession, TutorScope } from "../types";
...
export function boardLessonPrompt(session: LearningSession, scope: TutorScope, suggestion: BoardSuggestion, recentDialogue: BoardConversationMessage[] = []): string {
  ...
  const blueprint = directBoardBlueprint(session, scope, recentDialogue);
  return JSON.stringify({
    task: "把当前题目重构为一页独立可学的板书课程，不复制对话正文", discipline: session.problem.subject,
    boardBlueprint: blueprint.map(({ id, label, role, purpose, selfCheck }) => ({ move: id, label, role, purpose, why: nativeMoveWhy(label, role), selfCheck })),
