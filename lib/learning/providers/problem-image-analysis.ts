...
export function parseAuditedProblemSolution(value: JsonObject, requireModelConfidence = true, expectedAffectsSolving = false): { solution: ReturnType<typeof parseProblemSolution>; visualContext: ProblemVisualContext } {
  return { solution: parseProblemSolution(value), visualContext: parseAuditedVisualContext(value.visualContext, requireModelConfidence, expectedAffectsSolving) };
}

export function parseAuditedVisualContext(value: unknown, requireModelConfidence = true, expectedAffectsSolving = false): ProblemVisualContext {
  const visualContext = parseProblemVisualContext(value);
