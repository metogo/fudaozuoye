import type { SubjectBoardProfile } from "../board-subject-engine";
import type { BoardConversationMessage, BoardScene } from "../types";

export function assertNativeSceneSet(scenes: BoardScene[], context: BoardConversationMessage[], profile: SubjectBoardProfile) {
  if (scenes.length < 2 || scenes.length > 6) throw new Error("新版板书必须包含 2 到 6 个职责完整的教学单元");
  const roles = scenes.map((scene) => scene.role!);
  for (const required of ["orient", "reason"] as const) if (!roles.includes(required)) throw new Error(`新版板书缺少 ${required} 教学职责`);
  ...
  if (moveIndexes.some((index) => index < 0) || new Set(moveIndexes).size !== moveIndexes.length
    || moveIndexes.some((index, position) => position > 0 && index <= moveIndexes[position - 1])) throw new Error("新版板书教学动作必须按当前学科蓝图顺序出现且不能重复");
  ...
  for (const scene of scenes) for (const message of context) {
...
function assertDistinct(values: string[], label: string) {
...
function compact(value: string): string { return value.normalize("NFKC").replace(/[\s，。；：、“”‘’（）()\[\]【】]/g, "").toLowerCase(); }
