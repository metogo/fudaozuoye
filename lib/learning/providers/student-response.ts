...
type TextRequest = (system: string, prompt: string) => Promise<string>;

export async function transcribeStudentResponse(taskPrompt: string, request: TextRequest): Promise<{ text: string; confidence: number }> {
  ...
  const parse = (raw: string) => {
    ...
    if (!text || text.length > 2_000 || confidence < 0 || confidence > 1) throw new Error("没有可靠读到学生作答");
  ...
  try { return parse(first); } catch {
    try {
      ...
      throw providerError("暂时没能稳定读出这份作答，请保留图片并重试，或改用键盘输入", 502);
    }
  }
}
