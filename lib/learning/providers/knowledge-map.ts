...
import { splitEvidence } from "../evidence-segments";
import type { LearningSession } from "../types";
import { teachingAccuracyInstruction } from "./teaching-accuracy";

export const knowledgeMapRules = `${teachingAccuracyInstruction}
...
export const knowledgeDetailSystem = `${teachingAccuracyInstruction}
...
export function knowledgeEvidenceSegments(session: LearningSession) {
  return splitEvidence(mapEvidence(session)).map((text, i) => ({ id: `e${i + 1}`, text }));
}
