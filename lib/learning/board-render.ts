import type { BoardConceptVisual, BoardFunctionVisual, BoardGeometryVisual } from "./types";

export function conceptGraphDefinition(visual: BoardConceptVisual): string {
...
export function functionBoundingBox(visual: BoardFunctionVisual): [number, number, number, number] {
...
export function geometryBoundingBox(visual: BoardGeometryVisual): [number, number, number, number] {
  ...
  const radii = visual.objects.flatMap((object) => {
    ...
    if (object.radius) return [object.radius];
...
export function geometryPointPositions(visual: BoardGeometryVisual): Record<string, { x: number; y: number }> {
