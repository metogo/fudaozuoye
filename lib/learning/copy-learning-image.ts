...
function snapshotLearningText(source: HTMLElement): HTMLElement {
...
export function checkLearningImageSize(width: number, height: number): void {
...
async function renderLearningImage(source: HTMLElement, signal: AbortSignal): Promise<Blob> {
  ...
  signal.throwIfAborted();
  if (!blob || !blob.size) throw new Error("图片生成失败，请重试");
  return blob;
}

function generateLearningImage(snapshot: HTMLElement): Promise<Blob> {
...
export async function copyLearningImage(source: HTMLElement): Promise<void> {
