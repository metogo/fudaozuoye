...
export function parseSession(value: unknown): LearningSession {
  ...
  if (session.flow?.activeGate?.kind === "step_answer") {
    ...
    if (!session.stepCheck || session.stepCheck.type !== "short_text" || !session.stepCheck.prompt || !session.stepCheck.answer || !blank || typeof blank.before !== "string" || typeof blank.after !== "string" || typeof blank.hint !== "string") throw new Error("步骤填空数据不完整，请重新进入当前步骤");
    const gate = session.flow.activeGate;
    if (!gate.options?.some((option) => option.id === "view_step_answer")) gate.options = [...(gate.options ?? []), { id: "view_step_answer", label: "查看这个空的答案", emphasis: "secondary" }];
  }
