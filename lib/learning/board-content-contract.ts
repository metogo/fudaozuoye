import type { BoardBlock, Subject } from "./types";

const subjectSignals: Record<Subject, RegExp> = {
...
export function assertEnhancedBoardContent(subject: Subject, content: string, purpose: string, evidence: string, evidenceUniverse = evidence) {
  const remainder = withoutCanonical(content, purpose, evidence);
  if (genericTemplate.test(remainder)) throw new Error("增强板书正文不能使用通用模板代替当前学科动作");
  if (unsupportedClaim.test(remainder)) throw new Error("增强板书正文包含原题证据没有支持的事实断言");
  for (const claim of remainder.match(assertedOutcome) ?? []) if (!evidenceUniverse.includes(claim)) throw new Error("增强板书正文不能补充原题证据未支持的结果性陈述");
  const evidenceNumbers = new Set(numericTokens(evidenceUniverse));
  for (const number of numericTokens(remainder)) if (!evidenceNumbers.has(number)) throw new Error("增强板书正文不能补充原题没有给出的数字事实");
  ...
  if (unsupportedSentence) throw new Error(`增强板书正文只能组织学习动作，事实内容必须保留在逐字证据中：${unsupportedSentence}`);
  const signals = new Set(remainder.match(subjectSignals[subject]) ?? []);
  if (signals.size < 1) throw new Error("增强板书正文没有落实当前学科的证据与推理动作");
}

export function enhancedBoardInstructionSignature(content: string, purpose: string, evidence: string): string {
...
export function assertNativeMathBoardContent(
  blocks: BoardBlock[],
  expected: { relationExpressions: string[]; derivationExpressions: string[] },
) {
  if (blocks.length !== 5) throw new Error("数学原生板书必须完整覆盖五个推导动作");
  const signatures = blocks.map((block) => compactMathContent(block.content));
  if (new Set(signatures).size !== signatures.length) throw new Error("数学原生板书五段内容不能重复");
  const relationText = blocks.slice(0, 2).map((block) => block.content).join("\n");
  const derivationText = blocks[2]?.content ?? "";
  if (expected.relationExpressions.some((expression) => !relationText.includes(expression))) throw new Error("数学原生板书没有完整建立题干关系");
  if (expected.derivationExpressions.some((expression) => !derivationText.includes(expression))) throw new Error("数学原生板书没有形成有依据的关系推进");
  if (!/(?:检验|检查|复核)/.test(blocks[3]?.content ?? "") || !/(?:迁移|换字母|换数值|重建)/.test(blocks[4]?.content ?? "")) throw new Error("数学原生板书缺少可执行复核或题型迁移");
}

function withoutCanonical(content: string, purpose: string, evidence: string): string {
...
function compactMathContent(value: string): string {
