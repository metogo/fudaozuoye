...
export function assertEnhancedBoardContent(subject: Subject, content: string, purpose: string, evidence: string, evidenceUniverse = evidence) {
  ...
  for (const number of remainder.match(/-?\d+(?:\.\d+)?/g) ?? []) {
    if (!new RegExp(`(?<!\\d)${escapePattern(number)}(?!\\d)`).test(evidenceUniverse)) throw new Error("增强板书正文不能补充原题没有给出的数字事实");
  }
  ...
  if (signals.size < 2) throw new Error("增强板书正文没有落实当前学科的证据与推理动作");
}
