...
export interface TeachingScene {
  version: 1;
  template: "rectangle" | "groups" | "sharing" | "rate" | "comparison" | "fraction";
  worldId: string;
  stageIds: string[];
...
export function teachingSceneSvg(scene: TeachingScene): string {
  const content = scene.shapes.map((shape) => {
    if (shape.kind === "label") return `<text x="${shape.x}" y="${shape.y}" text-anchor="middle" font-size="19" font-family="sans-serif" fill="#193d32">${escapeXml(shape.text)}</text>`;
