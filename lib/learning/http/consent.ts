...
import { listProviderAvailability, listReasoningAvailability } from "../providers/config";
...
export async function postConsent(request: Request): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ accepted: true, version: "guardian-v1", providers: listProviderAvailability(), reasoningLevels: listReasoningAvailability() }), { headers });
  } catch (error) {
