...
type ChronologyKind = "numeric" | "dynasty";

function chronologyValue(value: string): { kind: ChronologyKind; rank: number } | null {
...
export function sortChronology<T extends { time: string }>(items: T[]): T[] | null {
  const ranked = items.map((item, index) => ({ item, index, chronology: chronologyValue(item.time) }));
  if (ranked.some((entry) => entry.chronology === null)) return null;
  if (new Set(ranked.map((entry) => entry.chronology!.kind)).size !== 1) return null;
