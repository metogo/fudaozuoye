import { assertNativeMathBoardContent } from "./board-content-contract";
import { assertValidLatex } from "./providers/board-latex";
import type { BoardBlock, BoardFormulaVisual, BoardGeometryObject, BoardGeometryVisual, BoardSemanticVisual } from "./types";
...
export interface TriangleTrigBoardContent {
  kind: "triangle_trig_relation";
  evidence: string;
  triangle: [string, string, string];
  relationAngles: { left: string; cosine: string; target: string };
  sides: { left: string; cosine: string; target: string };
  targetSide: string;
  knownSides: ScalarCondition[];
  area?: ScalarCondition;
  expressions: {
    trig: string;
    sideRelation: string;
    areaRelation: string;
    cosineRelation: string;
    reducedRelation: string;
  };
  gaps: string[];
}

export function createMathBoardContent(problem: string): TriangleTrigBoardContent | null {
  const triangle = triangleVertices(problem);
  const relation = trigRelation(problem);
  if (!triangle || !relation || !relationFitsTriangle(triangle, relation) || !hasSideCorrespondence(problem, triangle)) return null;
  const sides = {
...
export function createMathBoardBlocks(content: TriangleTrigBoardContent): BoardBlock[] {
  ...
  assertNativeMathBoardContent(blocks, {
...
export function createMathBoardAids(content: TriangleTrigBoardContent): BoardSemanticVisual[] {
  return [triangleVisual(content), formulaVisual(content)];
}

function triangleVisual(content: TriangleTrigBoardContent): BoardGeometryVisual {
  ...
  const objects: BoardGeometryObject[] = [
...
function formulaVisual(content: TriangleTrigBoardContent): BoardFormulaVisual {
  return {
    kind: "formula_chain",
    title: "同一条关系怎样从角推进到边",
    evidence: content.evidence,
...
function triangleVertices(problem: string): [string, string, string] | null {
  const match = problem.match(/(?:△|\\triangle\s*|三角形\s*)([A-Z])\s*([A-Z])\s*([A-Z])/i);
  if (!match) return null;
  const labels = match.slice(1, 4).map((label) => label.toUpperCase()) as [string, string, string];
  return new Set(labels).size === 3 ? labels : null;
}

function trigRelation(problem: string): TriangleTrigBoardContent["relationAngles"] | null {
  ...
  if (!match || new Set(match.slice(1, 4)).size !== 3) return null;
  return { left: match[1], cosine: match[2], target: match[3] };
}

function relationFitsTriangle(triangle: [string, string, string], relation: TriangleTrigBoardContent["relationAngles"]): boolean {
  return [relation.left, relation.cosine, relation.target].every((angle) => triangle.includes(angle));
}

function hasSideCorrespondence(problem: string, triangle: [string, string, string]): boolean {
...
function targetSideFromProblem(problem: string, triangle: [string, string, string]): string | null {
  ...
  for (const symbol of triangle.map((angle) => angle.toLowerCase())) {
    ...
    if (new RegExp(`(?:角|∠)\\s*${symbol}(?:\\s*的)?(?:值|大小|度数|为多少)|${symbol}\\s*角|(?:求|计算)[^。；;？?]{0,6}(?:角|∠)\\s*${symbol}`, "i").test(questionClause)
      || new RegExp(`${angle}\\s*(?:的)?(?:值|度数|大小|角度|为多少)`).test(questionClause)) continue;
    const pattern = new RegExp(`(?:求|问|则|那么|计算)?[^。；;？?]{0,8}(?:边长|边)?\\s*${symbol}\\s*(?:的)?(?:长|长度|值|是多少|为多少)|(?:求|计算)\\s*(?:边长|边)?\\s*${symbol}(?![A-Za-z])`);
    if (pattern.test(questionClause)) return symbol;
  }
  return null;
}

function directSideConditions(problem: string, triangle: [string, string, string]): ScalarCondition[] {
  ...
  }).filter((condition): condition is ScalarCondition => condition !== null);
}

function areaCondition(problem: string, triangle: [string, string, string]): ScalarCondition | undefined {
  ...
  if (marker[1] && marker[1].toUpperCase() !== triangle.join("")) return undefined;
  ...
  if (!marker[1] && nearestTriangle && nearestTriangle !== triangle.join("")) return undefined;
  ...
  if (!marker[1] && /三角形/.test(newTriangleMentions)) return undefined;
  const raw = scalarAfter(problem.slice(marker.index + marker[0].length));
  const latex = raw ? scalarLatex(raw) : null;
  return raw && latex ? { symbol: "S", raw, latex } : undefined;
}

function scalarAfter(source: string): string | null {
  const candidate = source
    ...
    .replace(/(?:\$|\\\))\s*$/, "")
    .trim();
  if (!candidate || candidate.length > 36) return null;
  return /^[0-9A-Za-z+\-*/.√\\{}\s]+$/.test(candidate) ? candidate : null;
}

function scalarLatex(raw: string): string | null {
  let value = raw.replace(/\s+/g, "").replace(/^\$|\$$/g, "");
  value = value.replace(/√\{?([A-Za-z0-9]+)\}?/g, "\\sqrt{$1}");
  if (!/^[0-9A-Za-z+\-*/.\\{}]+$/.test(value) || !/\d/.test(value)) return null;
  const commandsRemoved = value.replace(/\\(?:sqrt|frac)/g, "");
  if (/[A-Za-z]/.test(commandsRemoved) || value.includes("-") || /^0+(?:\.0+)?$/.test(value)) return null;
  if (!value.startsWith("\\frac")) {
    const slash = topLevelSlash(value);
    if (slash > 0 && slash < value.length - 1) {
      if (/[*/]/.test(value.slice(slash + 1))) return null;
      value = `\\frac{${value.slice(0, slash)}}{${value.slice(slash + 1)}}`;
    }
  }
  if (!isPositiveScalarLatex(value)) return null;
  try {
    assertValidLatex(`$${value}$`, "题干数值");
    return value;
  } catch {
    return null;
  }
...
function isPositiveScalarLatex(value: string): boolean {
...
function evidenceBeforeQuestion(problem: string, targetSide: string): string {
...
function factEvidence(problem: string, targetSide: string): string {
...
function plainScalar(raw: string): string {
  return raw.replace(/\\frac\s*\{([^{}]+)\}\s*\{([^{}]+)\}/g, "$1/$2")
    .replace(/\\sqrt\s*\{([^{}]+)\}/g, "√$1")
    .replace(/[$\\{}]/g, "").replace(/sqrt/gi, "√").replace(/\s+/g, "").slice(0, 9);
}

function knownSideSummary(knownSides: ScalarCondition[]): string {
  return knownSides.map((side) => `$${side.symbol}=${side.latex}$`).join("、");
}

function mathBody(expression: string): string { return expression.replace(/^\$|\$$/g, ""); }

function block(index: number, label: string, content: string): BoardBlock {
