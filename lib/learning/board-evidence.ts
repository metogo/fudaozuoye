...
export function extractBoardEvidenceClauses(value: string): string[] {
  const normalized = value.replace(/^[“"'‘]|[”"'’]$/g, "").replace(/\s+/g, " ").trim();
  if (!normalized) return [];
  const wrapped = normalized.match(/^(?:(?:根据)?(?:材料|原文|题目|史料)|read|passage|text)[：:]\s*(.+)$/i);
  if (wrapped) return extractBoardEvidenceClauses(wrapped[1]);
  return normalized.split(/(?<=[。！？!?；;])|(?<=[A-Za-z0-9'”"’])\.(?=\s+[A-Z“"'‘])|\n+/).flatMap(extractSentenceEvidence);
}

export function isTaskInstructionText(value: string): boolean {
  return extractBoardEvidenceClauses(value).length === 0;
}

function extractSentenceEvidence(sentence: string): string[] {
  ...
  if (isTaskInstruction(directiveBody)) return [];
  ...
  if (((metaLabel.test(normalized) && !modalFact) || directiveLead.test(directiveBody)) && metaInstruction.test(directiveBody)) return [];
  ...
  if (!selfContainedStem && !hasVerifiableFact && commandActionSignal.test(normalized)) return [];
  const premises = factualPremisesBeforeQuestion(normalized);
  if (premises.length) return premises;
  if (isTaskInstruction(normalized) && !selfContainedStem) return [];
  if (questionSignal.test(normalized)) return selfContainedStem ? [cleanEvidence(normalized)] : [];
  if (isTaskInstruction(normalized)) return selfContainedStem ? [cleanEvidence(normalized)] : [];
...
function factualPremisesBeforeQuestion(value: string): string[] {
  ...
  for (let index = 1; index < parts.length; index += 1) {
    ...
    if (!questionSignal.test(suffix) && !isTaskInstruction(suffix)) continue;
  ...
  if (compactPremise) return [cleanEvidence(compactPremise)];
  const katexGivens = value.match(/\$[^$\n]{2,}\$/g) ?? [];
  if (questionSignal.test(value) && katexGivens.length) return katexGivens.map(cleanEvidence);
  const explicitGivens = value.match(/(?:当|若|给定|取)?\s*[A-Za-z][A-Za-z0-9()]*\s*=\s*[^，,。！？!?；;\s]{1,32}/g) ?? [];
  if (questionSignal.test(value) && explicitGivens.length) return explicitGivens.map(cleanEvidence);
  return [];
}

function isTaskInstruction(value: string): boolean {
  const normalized = value.replace(/[。！？!?；;]+$/, "").trim();
  if (/^(?:分析表明|研究表明|统计表明|(?:分析|判断|计算)(?:结果|报告)(?:表明|显示|等于)|说明书指出)/.test(normalized)) return false;
  if (englishYouDirective.test(normalized)) return true;
  if (englishImperativeClause.test(normalized)) return true;
  if (englishTaskLead.test(normalized) && (!englishAmbiguousTaskLead.test(normalized) || !englishFactPredicate.test(normalized))) return true;
  if (/(?:作一评价|展开分析|说一说|谈一番|谈及|加以说明|进行评价|作答|作简要分析)/.test(normalized)) return true;
  ...
  if (frame.test(withoutCourtesy) && !withoutCourtesy.replace(frame, "").trim()) return true;
  const core = withoutCourtesy.replace(frame, "");
  if (chineseTaskLead.test(core)) return true;
