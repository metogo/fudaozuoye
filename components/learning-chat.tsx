...
const RectangleExperiment = createOptionalLearningFeature<ComponentProps<typeof import("./rectangle-experiment").RectangleExperiment>>(() => import("./rectangle-experiment").then(module => ({ default: module.RectangleExperiment })), "小实验");
...
export function LearningChat(props: LearningChatProps) {
  ...
  const seenSuggestionGroups = useRef(new Set<string>());
  ...
  const suggestionKey = JSON.stringify([props.session?.requestId, activeSuggestions.map((item) => [item.id, item.text])]);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const dock = composerDockRef.current?.getBoundingClientRect();
      const bottom = Math.min(bounds.bottom, window.innerHeight, dock && dock.height > 0 ? dock.top : bounds.bottom);
      const visibleHeight = Math.min(bottom, question.bottom) - Math.max(bounds.top, question.top, 0);
      if (!quote && !knowledgeMapOpen && question.height > 0 && visibleHeight >= Math.min(question.height, 60)) seenSuggestionGroups.current.add(suggestionKey);
      setSuggestionsBelow(!seenSuggestionGroups.current.has(suggestionKey) && isSuggestionBelowViewport(bottom, question.top, question.height));
    };
  ...
  }, [suggestionKey, showSuggestions, quote, knowledgeMapOpen]);
  ...
  useQuoteScrollDismiss(Boolean(quote), composerRef, dismissQuoteOnScroll);
  const cancelQuote = () => {
    const area = scrollRef.current;
  ...
  return <main className={`${headerStyles.shell} learning-chat-shell mx-auto flex h-dvh w-full max-w-3xl flex-col overflow-hidden ${isHome ? "home-chat-shell" : "lesson-chat-shell bg-[#f7f6f2]"}`}>
    ...
    <div ref={scrollRef} tabIndex={-1} aria-label={t("对话内容")} onScroll={updateScrollState} className={`chat-scroll min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 sm:px-6 ${isHome ? "home-chat-scroll pb-5 pt-0" : "pb-7 pt-5"}`}>
      {isHome ? <EmptyConversation statisticsEnabled={props.statisticsEnabled} ready={props.ready} fileError={fileError} motionPaused={props.busy || props.homeMotionPaused}/> : <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
        ...
        {initialReading ? <FirstExplanationPending label={props.loadingLabel} text={visibleChatMessages.find(message => message.role === "assistant" && message.kind === "assistant" && message.status === "streaming")?.text}/> : <ChatThinking active={props.busy && !hasActiveChatStream && !hasAssistantOutputForCurrentTurn} label={props.loadingLabel}/>}
        ...
        {props.session && !props.reviewProblem && !hasPendingRetry && gate && ["understanding", "solution_review", "post_solution"].includes(gate.kind)
          && props.session.flow.pathNodeIds.length === 0 && supportsRectangleExperiment(props.session.problem)
          && visibleChatMessages.some(message => message.role === "assistant" && message.kind === "assistant" && message.status === "complete")
          && <RectangleExperiment key={props.session.requestId} disabled={props.busy} onAsk={props.onQuestion}/>}
...
{quote && <div className="chat-quote-preview mx-auto mb-2 flex max-w-2xl gap-2 rounded-xl border border-emerald-900/10 bg-emerald-50/70 px-3 py-2" aria-label={t("正在引用的文字")}><div className="min-w-0 flex-1"><p className="mb-1 text-xs font-semibold text-emerald-800">{t("针对这段文字提问")}</p><div className="chat-quote-preview__body"><RichLearningText text={quote} compact/></div></div><button type="button" aria-label={t("取消引用")} className="chat-quote-preview__close" title={t("取消引用")} onClick={cancelQuote}><span aria-hidden="true">×</span></button></div>}
      {shouldShowChatJump(isHome, Boolean(quote), hasNewContent, showSuggestionHint) && <button type="button" onClick={() => {
        ...
        {t(chatJumpLabel(hasNewContent, showSuggestionHint))}
      </button>}
      {isHome && <div className="home-entry-actions mx-auto max-w-2xl">
        <label className={`home-camera-action ${!props.ready ? "is-unavailable" : ""}`}>
          <span className="home-camera-copy"><span>{t("拍照发题")}</span><ArrowIcon/></span><span className="home-camera-lens"><CameraIcon/></span>
          <input aria-label={t("拍照发题")} disabled={!props.ready} type="file" accept="image/*" capture="environment" className="sr-only" onChange={fileChange}/>
        </label>
        <div className="home-secondary-actions">
          <label className={`home-secondary-action ${!props.ready ? "is-unavailable" : ""}`}>
            <ImageIcon className="h-6 w-6"/><span>{t("从相册选择题目")}</span><ChevronIcon className="home-action-chevron"/>
            <input aria-label={t("从相册选择题目")} disabled={!props.ready} type="file" accept="image/*" className="sr-only" onChange={fileChange}/>
          </label>
          <button type="button" aria-label={t("白板写题")} className="home-secondary-action" disabled={!props.ready} onClick={() => props.onWhiteboard("question")}><PencilIcon className="h-6 w-6"/><span>{t("白板写题")}</span><ChevronIcon className="home-action-chevron"/></button>
        </div>
      </div>}
      ...
      <div className={`composer-surface mx-auto flex max-w-2xl flex-col gap-2 rounded-[22px] border border-stone-200 bg-white p-2 shadow-[0_10px_30px_rgba(41,37,36,.1)] focus-within:border-stone-400 ${isHome ? "home-input-panel" : ""}`}>
        <div className="chat-composer__input-row flex w-full items-end gap-1">
          ...
          {canAttach && !isHome && <div className="flex shrink-0 items-center">
            <label aria-label={t("拍照发题")} className={`flex h-11 w-11 items-center justify-center rounded-xl text-stone-500 transition hover:bg-stone-100 ${props.ready ? "cursor-pointer" : "cursor-not-allowed opacity-35"}`}><CameraIcon className="h-5 w-5"/><input disabled={!props.ready} type="file" accept="image/*" capture="environment" className="sr-only" onChange={fileChange}/></label>
            <label aria-label={t("从相册选择题目")} className={`flex h-11 w-11 items-center justify-center rounded-xl text-stone-500 transition hover:bg-stone-100 ${props.ready ? "cursor-pointer" : "cursor-not-allowed opacity-35"}`}><ImageIcon className="h-5 w-5"/><input disabled={!props.ready} type="file" accept="image/*" className="sr-only" onChange={fileChange}/></label>
            <button type="button" aria-label={t("白板写题")} title={t("白板写题")} disabled={!props.ready} onClick={() => props.onWhiteboard("question")} className="flex h-11 w-11 items-center justify-center rounded-xl text-stone-500 transition hover:bg-stone-100 hover:text-stone-900 disabled:cursor-not-allowed disabled:opacity-35"><PencilIcon className="h-5 w-5"/></button>
          </div>}
          {!isHome && showResponseTools && <div aria-label={t("输入方式")} className="flex shrink-0 items-center border-r border-stone-100 pr-1">
            <button type="button" disabled={props.busy} onClick={() => props.onWhiteboard(responseIntent)} aria-label={t(responseIntent === "answer" ? "打开白板作答" : "打开白板提问")} title={responseIntent === "answer" ? t("白板作答") : t("白板提问")} className="flex h-10 w-10 items-center justify-center rounded-xl text-stone-500 transition hover:bg-stone-100 hover:text-stone-900 disabled:opacity-35"><PencilIcon className="h-4 w-4"/></button>
            <label aria-label={t(responseIntent === "answer" ? "拍照作答" : "拍照提问")} title={responseIntent === "answer" ? t("拍照作答") : t("拍照提问")} className={`flex h-10 w-10 items-center justify-center rounded-xl text-stone-500 transition hover:bg-stone-100 hover:text-stone-900 ${props.busy ? "pointer-events-none opacity-35" : "cursor-pointer"}`}><CameraIcon className="h-4 w-4"/><input type="file" accept="image/*" capture="environment" className="sr-only" onChange={responsePhotoChange}/></label>
          </div>}
          <textarea ref={textareaRef} value={input} onChange={(event) => setInput(event.target.value)} rows={1} maxLength={questionMode ? 300 : answerMode ? 2_000 : props.session ? 300 : 8_000} disabled={props.busy || hasPendingRetry || Boolean(props.reviewProblem) || (choiceAnswerMode && !questionMode)} placeholder={hasPendingRetry ? t("请先重试刚才未完成的步骤") : quote ? t("想问这段文字的什么？") : questionMode ? t(currentTask?.placeholder ?? "") : choiceAnswerMode ? t("请点击上方选项作答") : t(currentTask?.placeholder ?? composerPlaceholder(props.session, props.ready))} aria-label={questionMode ? t("询问当前步骤") : answerMode ? t("输入你的答案") : t("输入题目或问题")} className="min-h-11 min-w-0 flex-1 resize-none overflow-y-auto bg-transparent px-2 py-2.5 text-sm leading-6 outline-none placeholder:text-stone-400 disabled:opacity-50"/>
          <button type="submit" disabled={props.busy || hasPendingRetry || !input.trim() || Boolean(props.reviewProblem) || (choiceAnswerMode && !questionMode) || (!props.session && !props.ready)} aria-label={questionMode ? t("发送问题") : answerMode ? t("提交答案") : t("发送")} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-stone-950 text-white transition active:scale-95 disabled:bg-stone-200 disabled:text-stone-400"><SendIcon className="h-5 w-5"/></button>
        </div>
      </div>
...
function ReasoningLevelPicker({ levels, level, onLevel }: { levels: ReasoningAvailability[]; level: ReasoningLevel; onLevel: (level: ReasoningLevel) => void }) {
  const t = useUiText();
  return <div className="flex min-h-11 w-full items-center justify-between gap-2 px-1"><span className="pl-1 text-[9px] font-semibold tracking-[.1em] text-stone-400">{t("推理强度")}</span><div className="flex items-center gap-0.5">{levels.map((item) => <button type="button" key={item.id} onClick={() => onLevel(item.id)} disabled={!item.available} aria-pressed={level === item.id} title={t(item.available ? `${item.label}推理` : `${item.label}推理尚未配置`)} className={`flex min-h-11 items-center gap-1.5 rounded-full px-3 text-[10px] font-semibold transition active:scale-[.98] ${level === item.id ? "bg-stone-100 text-stone-900" : "text-stone-500 hover:text-stone-700 disabled:opacity-35"}`}><span className={`h-1.5 w-1.5 rounded-full ${level === item.id ? "bg-emerald-500" : "bg-stone-300"}`}/>{t(item.label)}</button>)}</div></div>;
}

const MessageList = memo(function MessageList({ messages, initialReading, originalQuestionId, solutionDisplay, retryMessageId, busy, onRetry }: { messages: ChatMessage[]; initialReading: boolean; originalQuestionId?: string; solutionDisplay: "normal" | "locked"; retryMessageId?: string | null; busy: boolean; onRetry?: () => void }) {
  const firstLesson = messages.findIndex(message => message.role === "assistant" && message.kind === "assistant" && message.status !== "error" && hasExplanationBody(message.text));
  return messages.filter((message, index) => message.id !== originalQuestionId && !(initialReading && message.role === "assistant" && message.status === "streaming" && !hasExplanationBody(message.text)) && !(originalQuestionId && (initialReading || index < firstLesson)
    && message.kind === "milestone" && message.status !== "error"
    && ["题目已经读懂，先从核心思路开始", "先抓住这道题的核心"].includes(message.text))).map((message) => <div key={message.id} className={`chat-message-entry chat-message-entry--${message.role} ${message.id === messages[firstLesson]?.id ? readingStyles.arrival : ""}`}>
    ...
    {message.id === messages[firstLesson]?.id && message.status === "complete" && !busy && <SelectionHint/>}
...
const MessageBubble = memo(function MessageBubble({ message, solutionDisplay, onRetry, retryBusy }: { message: ChatMessage; solutionDisplay: "normal" | "locked"; onRetry?: () => void; retryBusy?: boolean }) {
  ...
  if (message.role === "assistant" && message.scopeLabel === "原题完整讲解") {
    if (message.status === "error") return <div className="rounded-2xl border border-red-100 bg-red-50/60 px-4 py-3"><p className="text-xs font-semibold text-red-800">{t("完整讲解未完成")}</p><MessageFailure onRetry={onRetry} busy={retryBusy}/></div>;
    if (solutionDisplay === "locked") return <div className="rounded-2xl border border-stone-200 bg-white px-4 py-3"><p className="text-[10px] font-semibold tracking-[.1em] text-amber-700">{t("完整讲解已收起")}</p><p className="mt-1 text-xs leading-5 text-stone-500">{t("接下来只保留学习任务，避免答案继续影响独立思考。")}</p></div>;
  }
  ...
  return <article className={`chat-message flex ${mine ? `chat-message--student ${message.reference ? "chat-message--referenced " : ""}justify-end` : "chat-message--teacher justify-start"}`}>
    <div className={`max-w-[90%] ${mine ? "rounded-[22px_22px_6px_22px] bg-stone-950 text-white" : "rounded-[6px_22px_22px_22px] border border-stone-200 bg-white text-stone-900 shadow-sm"} px-4 py-3`}>
      {mine && message.reference && <blockquote className="chat-message-reference"><p className="chat-message-reference__label">{`${t("引用 ·")} ${message.reference.scopeLabel}`}</p><p className="chat-message-reference__summary">{message.reference.sourceSummary}</p></blockquote>}
      {message.scopeLabel && !mine && <div className="mb-1.5 text-[9px] font-semibold tracking-[.1em] text-amber-700">{t("关于「")}<RichLearningText text={message.scopeLabel} compact/>{t("」")}</div>}
      {message.imageUrl && <img src={message.imageUrl} alt={t("学生发送的题目")} className="mb-3 max-h-56 w-full rounded-xl object-contain"/>}
      {mine
...
function MessageFailure({ onRetry, busy, mine }: { onRetry?: () => void; busy?: boolean; mine?: boolean }) {
  ...
  return <div className={`chat-message-failure ${mine ? "chat-message-failure--student" : ""}`}>
    <span>{onRetry ? t("本条处理未完成") : t("本条处理未完成，请重新发送问题或继续当前步骤。")}</span>
    {onRetry && <button type="button" className="chat-message-retry" disabled={busy} onClick={onRetry} aria-label={t("重试这条消息")} title={t("重新请求，不刷新页面")}><RefreshIcon/><span>{busy ? t("重试中") : t("重试")}</span></button>}
  </div>;
}
...
function SuggestedQuestionTrail({ suggestions, onSuggestion }: { suggestions: SuggestedQuestion[]; onSuggestion: (suggestion: SuggestedQuestion) => void }) {
  const t = useUiText();
  return <section className="suggested-question-trail mt-4" aria-label={t("猜你想问")}>
    ...
    <div className="suggested-question-trail__branches">
      {suggestions.map((suggestion, index) => <button type="button" key={suggestion.id} onClick={() => onSuggestion(suggestion)} className="suggested-question-trail__question group">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-1 block truncate text-[9px] font-medium text-stone-400">{t("关联 ·")}{suggestion.scopeLabel}</span>
        </span>
...
function GateCard({ gate, busy = false, onTranscribeStep, answerChoices, choicesDerivedFromPrompt, allowFullSolution, onChoice, onAnswer, knowledgeMapAction }: { gate: LearningGate; busy?: boolean; onTranscribeStep?: LearningChatProps["onTranscribeStep"]; answerChoices?: string[]; choicesDerivedFromPrompt: boolean; allowFullSolution: boolean; illustrationAvailability: IllustrationAvailability; onChoice: (gate: LearningGate, choice: LearningChoice) => void; onAnswer: (answer: string) => void; knowledgeMapAction?: ReactNode }) {
  ...
  return <section data-gate-kind={gate.kind} className="chat-gate rounded-[24px] border border-stone-200 bg-white p-4 shadow-[0_14px_36px_rgba(41,37,36,.1)]" aria-label={t("当前学习任务")}>
    <div className="mb-3 flex items-start gap-3"><span className="chat-gate__avatar flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-stone-950 text-white"><TutorIcon className="h-4 w-4"/></span><div><p className="text-[9px] font-semibold tracking-[.14em] text-stone-400">{t("轮到你了")}</p><h2 className="mt-1 text-sm font-bold leading-6"><RichLearningText text={t(gate.title)} compact/></h2></div></div>
    <div className="chat-gate__body">
    {parsedPrompt?.body && gate.kind !== "step_answer" && <div className="learning-prompt mb-3 rounded-2xl border border-stone-200/80 bg-stone-50 px-4 py-4 text-stone-800"><p className="mb-2 text-[9px] font-bold tracking-[.14em] text-stone-400">{gate.kind === "original_answer" ? t("同一道原题") : gate.kind === "transfer_answer" ? t("同类练习") : gate.kind === "solution_recall_answer" ? t("关键步骤检查") : t("理解检查")}</p><RichLearningText text={parsedPrompt.body}/></div>}
    {gate.kind === "step_answer" && gate.stepBlank ? <StepBlank key={gate.id} gate={gate} busy={busy} onTranscribe={onTranscribeStep} onContinue={() => onChoice(gate, "continue")} onReveal={() => onChoice(gate, "view_step_answer")} onHint={() => onChoice(gate, "not_understood")}/> : isAnswerGate ? <>
      {mainOptions.some((option) => option.id === "not_understood") && <button type="button" onClick={() => onChoice(gate, "not_understood")} className="mb-3 min-h-11 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm font-semibold text-stone-700">{t("还没懂，再讲一下")}</button>}
...
function AnswerChoices({ choices, stripLabels, onAnswer }: { choices?: string[]; stripLabels: boolean; onAnswer: (answer: string) => void }) {
  ...
  return <div className="mb-2 space-y-2" aria-label={t("答案选项")}><p className="text-[10px] font-semibold text-stone-400">{t("选择一个答案")}</p>{choices.map((choice, index) => {
...
function RecognitionReview({ problem, onConfirm, busy }: { problem: ProblemSnapshot; onConfirm: (problem: ProblemSnapshot) => void; busy: boolean }) {
  ...
  return <section className="recognition-review rounded-[24px] border border-amber-200 bg-amber-50/70 p-4">
    <p className="text-[10px] font-semibold tracking-[.12em] text-amber-800">{t("识别结果需要你确认")}</p>
    <label className="mt-3 block text-[11px] font-semibold text-stone-600">{t("题目文字")}</label>
    ...
    {problem.visualContext ? <div className="mt-3 rounded-2xl border border-amber-200 bg-white p-3">
      <p className="text-[11px] font-semibold text-stone-700">{t("这道题需要看图吗？")}</p>
      <div className="mt-2 grid grid-cols-3 gap-2">
        <button type="button" aria-pressed={visualUse === "solving"} onClick={() => setVisualUse("solving")} className={`min-h-10 rounded-xl text-[11px] font-semibold ${visualUse === "solving" ? "bg-stone-950 text-white" : "border border-stone-200 text-stone-600"}`}>{t("解题需要")}</button>
        <button type="button" aria-pressed={visualUse === "context"} onClick={() => setVisualUse("context")} className={`min-h-10 rounded-xl text-[11px] font-semibold ${visualUse === "context" ? "bg-stone-950 text-white" : "border border-stone-200 text-stone-600"}`}>{t("辅助理解")}</button>
        <button type="button" aria-pressed={visualUse === "unrelated"} onClick={() => setVisualUse("unrelated")} className={`min-h-10 rounded-xl text-[11px] font-semibold ${visualUse === "unrelated" ? "bg-stone-950 text-white" : "border border-stone-200 text-stone-600"}`}>{t("与题无关")}</button>
      </div>
      {visualUse === "solving" ? <>
      <p className="mt-3 text-[11px] font-semibold text-stone-700">{t("图中信息")}</p>
      <p className="mt-1 text-[10px] leading-5 text-stone-500">{t("每行一条，只保留属于这道题、且能从图中直接看到的条件。")}</p>
      <textarea aria-label={t("图中信息")} value={visualFacts} onChange={(event) => setVisualFacts(event.target.value)} rows={Math.max(3, Math.min(6, factLines.length + 1))} className="mt-2 w-full resize-y rounded-xl border border-stone-200 bg-stone-50 p-3 text-xs leading-6 outline-none focus:border-amber-500"/>
      {missingRequiredVisualFacts ? <p className="mt-2 text-[10px] font-medium text-red-600">{t("这道题依赖配图，请补全图中条件或重新拍摄。")}</p> : null}
      </> : null}
    </div> : null}
    <button type="button" disabled={busy || text.trim().length < 3 || Boolean(missingRequiredVisualFacts)} onClick={confirm} className="mt-3 min-h-11 w-full rounded-xl bg-stone-950 text-sm font-semibold text-white disabled:opacity-35">{t("确认题目，开始讲解")}</button>
  </section>;
}

function NextTurnPlaceholder() {
  const t = useUiText();
  return <section className="next-turn-placeholder chat-gate rounded-[24px] border border-stone-200 bg-white p-4 shadow-[0_14px_36px_rgba(41,37,36,.08)]" role="status" aria-live="polite" aria-label={t("下一步正在准备")}>
    <div className="flex items-start gap-3">
      <CommaCompanion thinking/>
      <div className="min-w-0 flex-1"><div className="next-turn-placeholder__eyebrow"><p className="text-[9px] font-semibold tracking-[.14em] text-stone-400">{t("下一步正在准备")}</p><span className="next-turn-placeholder__status">{t("即将出现")}</span></div><h2 className="mt-1 text-sm font-bold leading-6 text-stone-800">{t("接下来会轮到你")}</h2><p className="mt-1 text-[10px] leading-5 text-stone-400">{t("正在把刚才的内容整理成下一步互动")}</p></div>
    </div>
...
function CompletionActions({ session, onTransfer, onNew }: { session: LearningSession; onTransfer: () => void; onNew: () => void }) {
  ...
  return <section className="completion-panel completion-panel--mastered rounded-[24px] bg-stone-950 p-5 text-white"><p className="text-[10px] font-semibold tracking-[.14em] text-emerald-400">{t("已验证掌握")}</p><h2 className="mt-2 text-xl font-bold tracking-[-.03em]">{t(title)}</h2><p className="mt-2 text-xs leading-5 text-stone-400">{t(evidence)}{session.flow.pathNodeIds.length ? ` ${t("这次还补过 {count} 个基础节点。", { count: session.flow.pathNodeIds.length })}` : ""} {t("还可以继续追问，不会丢失本题上下文。")}</p><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={onTransfer} className="min-h-11 rounded-xl bg-white text-xs font-semibold text-stone-950">{t("再练一道同类题")}</button><button type="button" onClick={onNew} className="min-h-11 rounded-xl border border-white/15 text-xs font-semibold">{t("开始新题")}</button></div><p className="mt-3 text-center text-[9px] text-stone-500">{t("同类题由 AI 生成，不冒充真题或高频题")}</p></section>;
}

function ReviewCompletionActions({ onRetryOriginal, onTransfer, onNew }: { onRetryOriginal: () => void; onTransfer: () => void; onNew: () => void }) {
  const t = useUiText();
  return <section className="completion-panel rounded-[24px] border border-stone-200 bg-white p-5 shadow-[0_14px_36px_rgba(41,37,36,.08)]"><p className="text-[10px] font-semibold tracking-[.14em] text-amber-700">{t("已学习 · 尚未验证掌握")}</p><h2 className="mt-2 text-xl font-bold tracking-[-.03em] text-stone-950">{t("关键步骤已经理解")}</h2><p className="mt-2 text-xs leading-5 text-stone-500">{t("你看过完整讲解，也说清了关键关系；但还没有独立做对一道题，因此暂不标记为“已掌握”。")}</p><div className="mt-4 grid gap-2"><button type="button" onClick={onRetryOriginal} className="min-h-11 rounded-xl bg-stone-950 px-3 text-xs font-semibold text-white">{t("遮住讲解，重做原题")}</button><button type="button" onClick={onTransfer} className="min-h-11 rounded-xl border border-stone-200 px-3 text-xs font-semibold text-stone-700">{t("换一道同知识点题")}</button><button type="button" onClick={onNew} className="min-h-11 rounded-xl px-3 text-xs font-semibold text-stone-400">{t("开始新题")}</button></div></section>;
}
