...
import type { BoardConversationMessage, BoardDocument, BoardExperience, BoardWorkspaceState } from "@/lib/learning/types";
...
import { BoardVisualBoundary } from "./board-visual-boundary";
...
export function BoardWorkspace({ document, experience, sourceMessages, state, onChange }: BoardWorkspaceProps) {
  const scenes = experience.scenes;
  const discipline = experience.subject;
  const stepSubject = experience.subject;
  ...
  return <div ref={workspaceRef} className={`board-workspace board-course${discipline ? ` board-course--${discipline}` : ""}`} data-discipline={discipline}>
    ...
    <div className="board-course-content">
      {scenes.map((scene, index) => {
        ...
        const annotations = experience.annotations.filter((annotation) => annotation.blockId === node.id);
        ...
        return <section key={node.id} data-board-step-index={index} className={`board-workspace-node board-course-unit board-workspace-node--${scene.tone}`} aria-labelledby={`board-node-${node.id}`}>
          <header>
            ...
            <div><p>{copy.eyebrow}</p><h2 id={`board-node-${node.id}`}>{copy.title}</h2></div>
          </header>
          ...
          {recalling ? <div className="board-recall-mask board-recall-mask--inline">
            ...
            {scene.actions.map((action) => scene.elements.find((element) => element.id === action.targetId)).filter(Boolean).map((element) => element!.type === "text"
              ? <div key={element!.id} className="board-workspace-node__content"><span>怎么做</span><MarkedBoardText content={element!.text} annotations={annotations}/></div>
              : <BoardVisualBoundary key={element!.id} visual={element!.visual} fallbackText={element!.fallbackText} primary={scene.medium !== "text"} purpose={scene.purpose}/>)}
            {scene.evidence && scene.evidence !== scene.visual?.evidence && <div className="board-teaching-evidence"><span>题目依据</span><RichLearningText text={scene.evidence} compact/></div>}
