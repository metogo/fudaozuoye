...
export function LearningWorkspace(props: WorkspaceProps) {
  ...
  return <main className="workspace-screen min-h-dvh pb-36">
    ...
    {props.notice && <div className="notice-banner mx-auto mt-4 flex max-w-6xl gap-2 px-5"><div className="flex w-full gap-2 rounded-xl border border-stone-300 bg-white px-3 py-2.5 text-xs text-stone-700"><InfoIcon className="h-4 w-4 shrink-0"/>{props.notice}</div></div>}
...
function NodeCoach({ node, actionable, answer, setAnswer, busy, onExpand, onVerify, onParentConfirm, onShowSolution, onReset }: { node: KnowledgeNode; actionable: boolean; answer: string; setAnswer: (value: string) => void; busy: boolean; onExpand: WorkspaceProps["onExpand"]; onVerify: WorkspaceProps["onVerify"]; onParentConfirm: WorkspaceProps["onParentConfirm"]; onShowSolution: () => Promise<void>; onReset: WorkspaceProps["onReset"] }) {
  ...
  return <div>
    ...
    {!isRoot && <div className="space-y-5 p-5 sm:p-7">
      <section><p className="mb-2 text-xs font-semibold text-stone-400">家长怎么讲</p><p className="text-[15px] leading-7 text-stone-800">{node.attempts > 0 && node.atomic ? node.teaching.alternateExplanation : node.teaching.explanation}</p></section>
      <section className="rounded-2xl bg-stone-100 p-4"><p className="mb-2 text-xs font-semibold text-stone-500">换个更简单的例子</p><p className="text-sm leading-6">{node.teaching.example}</p></section>
      <section className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-stone-200 p-4"><p className="mb-2 text-xs font-semibold text-stone-400">问孩子</p><p className="text-sm leading-6">“{node.teaching.parentPrompt}”</p></div><div className="rounded-2xl border border-stone-200 p-4"><p className="mb-2 text-xs font-semibold text-stone-400">真正理解的信号</p><p className="text-sm leading-6">{node.teaching.expectedSignal}</p></div></section>
      <div className="flex gap-2 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900"><InfoIcon className="mt-0.5 h-4 w-4 shrink-0"/><p>常见误区：{node.teaching.misconception}</p></div>
    </div>}
    ...
    <section className="border-t border-stone-100 p-5 sm:p-7">
      ...
      <div className="mb-4 flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-stone-900 text-white"><SparkIcon className="h-4 w-4"/></span><div><p className="text-xs font-semibold">{isRoot ? "独立重做" : "微型检查"}</p><p className="text-[10px] text-stone-400">由孩子作答，不提示答案</p></div></div>
