...
import type { BoardAnnotation, BoardLesson, ChatMessage } from "@/lib/learning/types";
...
export function LearningBoard({ lesson, messages, busy, loadingLabel, notice, retryLabel, onAsk, onClose, onRetry }: LearningBoardProps) {
  ...
  return <section ref={boardRef} role="dialog" aria-modal="true" aria-label={`AI 专注板书：${learningTextToPlainText(lesson.title)}`} tabIndex={-1} onKeyDown={handleBoardKeyDown} className="learning-board fixed inset-0 z-50 flex h-dvh flex-col overflow-hidden bg-[#f1f2ea] text-stone-950 outline-none">
    ...
    <div className="board-scroll min-h-0 flex-1 overflow-y-auto px-4 pt-6 sm:px-7 sm:pt-10" style={{ paddingBottom: questionDockHeight > 0 ? questionDockHeight + 44 : panelExpanded ? 280 : 112 }}>
      <article className="mx-auto w-full max-w-4xl">
        <div className="board-overview mb-7 max-w-2xl rounded-[20px] border border-emerald-950/10 bg-white/45 p-4"><p className="text-[9px] font-semibold tracking-[.14em] text-emerald-900">完整板书 · 关系总览</p><div className="mt-2 text-xs leading-6 text-stone-600"><RichLearningText text={lesson.subtitle} compact/></div>{annotations.length > 0 && <p className="mt-3 border-t border-emerald-950/10 pt-3 text-[10px] leading-5 text-stone-500">圈、线、框只标记模型判断的关键条件、核心关系和易错边界；每处标记下方都说明原因。</p>}</div>
        {lesson.visual && <BoardVisualFigure visual={lesson.visual}/>}
        <div className={`board-grid board-grid--${lesson.layout}`}>
    ...
    <div className={`pointer-events-none fixed inset-x-0 bottom-0 z-10 px-4 pb-[max(12px,env(safe-area-inset-bottom))] sm:px-7 ${panelExpanded ? "bg-gradient-to-t from-[#f1f2ea] via-[#f1f2ea]/96 to-transparent pt-12" : "pt-3"}`} style={{ bottom: keyboardInset }}>
      <div ref={questionDockRef} className={`board-question-dock pointer-events-auto mx-auto max-w-2xl border border-stone-900/10 bg-white/95 shadow-[0_18px_60px_rgba(28,25,23,.14)] backdrop-blur-xl ${panelExpanded ? "overflow-hidden rounded-[24px] p-3" : "rounded-2xl px-3 py-2"}`}>
