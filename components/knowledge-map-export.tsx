...
export function KnowledgeMapExport({ map, positions, complete, nodeTypes }: Snapshot & { nodeTypes: NodeTypes }) {
  ...
  return <>
    <button ref={buttonRef} className="knowledge-map-export-button" onClick={start} disabled={!map.nodes.length || Boolean(snapshot)} aria-label={t("导出图片")} title={t("导出全部已生成知识点、连线和关系标签")} aria-busy={Boolean(snapshot)}>{snapshot ? t("导出中…") : t("导出图片")}</button>
    {(error || result) && <div className={`knowledge-map-export-notice ${styles.exportNotice}`} role={error ? "alert" : "status"}>{error ? t(error) : <>{t("图片已生成 ·")}<a href={result!.url} download={result!.filename}>{t("再次下载")}</a></>}</div>}
...
function ExportSheet({ snapshot, nodeTypes, finish }: { snapshot: Snapshot; nodeTypes: NodeTypes; finish: (blob: Blob | null, message?: string) => void }) {
  ...
  const edges = useMemo(() => knowledgeMapEdges(snapshot.map.edges, null, t), [snapshot, t]);
  ...
  return <div className={styles.exportStage} aria-hidden="true" inert>
    <div ref={sheet} className={`knowledge-map-export-sheet ${styles.exportSheet}`} style={{ width: frame?.width ?? 1024 }}>
      <header><strong>{t("本题知识图谱")}</strong><span>{snapshot.complete ? t("共 {count} 个知识点", { count: nodes.length }) : t("生成中快照 · 已生成 {count} 个知识点", { count: nodes.length })}</span></header>
      ...
      <footer>{t("AI 整理，请结合原题理解")}{!snapshot.complete && t(" · 本图仅包含导出时已生成的内容")}</footer>
    </div>
  </div>;
}
