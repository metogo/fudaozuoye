...
export function knowledgeMapEdges(relations: MapRelation[], selected: string | null = null): Edge[] {
  return relations.map(e => {
    const active = e.from === selected || e.to === selected;
    return { id: `${e.from}:${e.to}`, source: e.from, target: e.to, type: "smoothstep", label: relationLabel[e.kind], animated: false,
