...
export type ConceptNode = Node<{ concept: MapConcept; root: boolean; expanded: boolean; childCount: number; dimmed: boolean; active?: boolean; pending?: boolean; preview?: boolean; continuing?: boolean; stopped?: boolean; toggle: (id: string) => void }, "concept">;
const Concept = memo(function Concept({ id, data, selected }: NodeProps<ConceptNode>) {
  const t = useUiText();
  if (data.pending) return <div key="pending" className={`knowledge-map-pending ${styles.node} ${styles.pendingNode} ${data.active ? styles.generating : ""}`} data-active={data.active || undefined} data-stopped={Boolean(data.stopped)} aria-label={data.stopped ? t("等待继续整理") : data.root ? t("正在整理本题核心") : t("正在补充知识点")}>
  ...
  return <div className="knowledge-map-concept" style={{ position: "relative" }}>
    <div key="ready" data-active={data.active || undefined} className={`${styles.node} ${data.root ? styles.core : ""} ${selected ? styles.selected : ""} ${data.dimmed ? styles.dimmed : ""} ${data.active ? styles.generating : ""}`}>
      <Handle type="target" position={Position.Top} isConnectable={false}/>
