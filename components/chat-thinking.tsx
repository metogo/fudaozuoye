...
export function ChatThinking({ active, label }: { active: boolean; label: string }) {
  ...
  return <div className={styles.shell} data-exiting={exiting} aria-hidden={!active}><div className={styles.clip}>
      <div className={styles.content} role="status" aria-live="polite" aria-atomic="true" aria-label="小逗号正在思考">
        <div className={styles.avatar} aria-hidden="true"><CommaCompanion thinking canCelebrate={false}/></div>
        <div className={styles.copy}><div className={styles.eyebrow}>小逗号正在思考<span className={styles.dots} aria-hidden="true"><i/><i/><i/></span></div><div key={content.title} className={styles.phase}><h3>{content.title}</h3><p>{content.description}</p></div><blockquote className={styles.quote} aria-label="学习寄语" aria-live="off"><p>“{quote.text}”</p><cite>— {quote.source}</cite></blockquote></div>
      </div>
    </div></div>;
}
