...
export function KnowledgeConnectionSlot({ entry, onOpen, onRetry }: { entry: ConnectionEntry; onOpen: (focus?: MapFocus) => void; onRetry: () => void }) {
  ...
  return <div ref={slot} className="knowledge-connection-slot" data-status={entry.status}>
    {entry.status === "pending" ? <div className={`knowledge-connection-pending ${styles.notice}`} role="status">正在整理知识连接 · 不影响继续学习</div>
      : entry.status === "error" ? <div className={`knowledge-connection-error ${styles.notice}`}><span>知识连接暂未整理成功</span><button type="button" className={styles.more} onClick={onRetry}>重试知识连接</button></div>
...
export function KnowledgeConnection({ connection, onOpen }: { connection: Connection; onOpen: (focus?: MapFocus) => void }) {
  ...
  return <aside className={`knowledge-connection ${styles.connection}`} aria-label="本段知识连接">
    <p className={styles.eyebrow}>把知识连起来</p>
    <div className={styles.pair}>
      {nodes.map((node, i) => <span className={styles.item} key={node.title}>
        {i === 1 && <span className={styles.arrow} aria-label={connection.kind === "prerequisite" ? "帮助理解" : "结合使用"}>{connection.kind === "prerequisite" ? "→" : "↔"}</span>}
    ...
    <p className={styles.hint}>点知识点，展开看看</p>
    {active && <div id={detailId} role="region" aria-label={`${active.title}的就地讲解`} className={`knowledge-connection__detail ${styles.detail}`}>
      <RichLearningText text={active.explanation}/>
      {active.example && <div className={styles.example}><strong>看个例子</strong><RichLearningText text={active.example}/></div>}
      <details className={`knowledge-connection__source ${styles.source}`}><summary>对应哪段讲解</summary><blockquote>{connection.anchor}</blockquote></details>
      <button type="button" className={styles.more} onClick={() => onOpen({ title: active.title })}>在完整图谱中查看 →</button>
    </div>}
  </aside>;
}
