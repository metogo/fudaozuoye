...
import { Background, Handle, Position, ReactFlow, applyNodeChanges, type Node, type NodeProps, type NodeChange, type ReactFlowInstance } from "@xyflow/react";
...
type ConceptNode = Node<{ concept: MapConcept; root: boolean; expanded: boolean; childCount: number; dimmed: boolean; pending?: boolean; stopped?: boolean; toggle: (id: string) => void }, "concept">;
const Concept = memo(function Concept({ id, data, selected }: NodeProps<ConceptNode>) {
  if (data.pending) return <div key="pending" className={`${styles.node} ${styles.pendingNode}`} aria-label={data.root ? "正在整理本题核心" : "正在补充知识点"}>
    <Handle type="target" position={Position.Top} isConnectable={false}/>
    <span className={styles.nodeLabel}>{data.root ? "本题核心" : "下一个知识点"}</span>
    <strong>{data.stopped ? "等待继续整理" : data.root ? "正在整理本题核心" : "正在连接知识…"}</strong>
    <span className={styles.skeletonLines} aria-hidden="true"><i/><i/></span>
  </div>;
  return <div key="ready" className={`${styles.node} ${data.root ? styles.core : ""} ${selected ? styles.selected : ""} ${data.dimmed ? styles.dimmed : ""}`}>
    <Handle type="target" position={Position.Top} isConnectable={false}/>
...
function MapCanvas({ generation, initialFocus }: { generation: ReturnType<typeof useKnowledgeMap>; initialFocus?: MapFocus }) {
  ...
  const map = generation.map ?? emptyMap;
  ...
  const selected = selection ? selection.id : findMapFocus(map.nodes, initialFocus)?.id ?? null;
  ...
  const readyNodes: ConceptNode[] = useMemo(() => map.nodes.filter(n => visible.has(n.id)).map(concept => ({ id: concept.id, type: "concept", position: placed[concept.id], measured: measured[concept.id], selected: concept.id === selected, ariaLabel: `${concept.title}，按回车查看，方向键移动`, data: { concept, root: concept.id === map.rootId, expanded: expanded.has(concept.id), childCount: map.edges.filter(e => e.from === concept.id).length, dimmed: Boolean(selected && !related.has(concept.id)), toggle } })), [map, placed, measured, expanded, selected, related, toggle, visible]);
  ...
  const edges = useMemo(() => knowledgeMapEdges(map.edges.filter(e => expanded.has(e.from) && visible.has(e.from) && visible.has(e.to)), selected), [map, selected, visible, expanded]);
  ...
  return <div className={styles.workspace}>
    <KnowledgeMapProgress count={map.nodes.length} total={plan?.nodes.length ?? (complete ? map.nodes.length : null)} complete={complete} error={error} latest={map.nodes.at(-1)?.title} retry={retry}/>
    <div ref={canvasRef} className={styles.canvas} aria-label="可拖拽的知识图谱" onKeyDown={event => { if (event.key === "Enter" && event.target instanceof HTMLElement && event.target.classList.contains("react-flow__node")) { const id = event.target.getAttribute("data-id"); if (id && map.nodes.some(n => n.id === id)) setSelected(id); } }}>
      <ReactFlow<ConceptNode> nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={changes} onNodeClick={(_, node) => { if (!node.data.pending) setSelected(node.id); }} onPaneClick={() => setSelected(null)} onNodeDragStop={() => { persist(); setAnnouncement("节点位置已保存"); }} onInit={instance => {
        setFlow(instance);
        if (!saved) {
      ...
      <div className={styles.tools} role="group" aria-label="图谱视图工具"><button aria-label="缩小图谱" onClick={() => void flow?.zoomOut({ duration: duration() })}>−</button><span>{zoom}%</span><button aria-label="放大图谱" onClick={() => void flow?.zoomIn({ duration: duration() })}>＋</button><i/><button disabled={!map.nodes.length} onClick={recoverView}>查看全图</button><button disabled={!map.nodes.length} onClick={() => { const arranged = plan ? slots : arrangeConcepts(map); positionsRef.current = arranged; setPositions(arranged); setAnnouncement("已恢复整齐布局，知识关系没有改变。"); recoverView(); }}>整理</button><KnowledgeMapExport map={map} positions={placed} complete={complete} nodeTypes={nodeTypes}/></div>
      <div className={styles.hint}>拖节点排布 · 拖空白移动 · 双指缩放</div>
    </div>
    {focus && <section className={styles.detail} aria-label={`${focus.title}的知识说明`}><header><div><span>知识卡片</span><h2>{focus.title}</h2></div><button onClick={() => setSelected(null)} aria-label="关闭知识卡片"><CloseIcon/></button></header><div className={styles.detailBody}><KnowledgeExplanation key={focus.id} focus={focus} map={map} stateToken={stateToken} cacheKey={cacheKey} partial={!complete}/>{focus.evidence && <blockquote><small>对应本题条件</small><RichLearningText text={focus.evidence}/></blockquote>}{map.edges.filter(e => e.from === focus.id || e.to === focus.id).map(e => <div className={styles.relation} key={`${e.from}:${e.to}`}><p><strong>{map.nodes.find(n => n.id === e.from)?.title}</strong> → <strong>{map.nodes.find(n => n.id === e.to)?.title}</strong></p><span>{relationLabel[e.kind]}</span><RichLearningText text={e.reason}/></div>)}</div></section>}
