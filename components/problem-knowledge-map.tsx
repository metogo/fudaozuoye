...
const Concept = memo(function Concept({ id, data, selected }: NodeProps<ConceptNode>) {
  const t = useUiText();
  if (data.pending) return <div key="pending" className={`${styles.node} ${styles.pendingNode}`} aria-label={data.root ? t("正在整理本题核心") : t("正在补充知识点")}>
    <Handle type="target" position={Position.Top} isConnectable={false}/>
    <span className={styles.nodeLabel}>{data.root ? t("本题核心") : t("下一个知识点")}</span>
...
function MapCanvas({ generation, initialFocus }: { generation: ReturnType<typeof useKnowledgeMap>; initialFocus?: MapFocus }) {
  ...
  const edges = useMemo(() => knowledgeMapEdges(map.edges.filter(e => expanded.has(e.from) && visible.has(e.from) && visible.has(e.to)), selected, t), [map, selected, visible, expanded, t]);
