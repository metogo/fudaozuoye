...
export function KnowledgeMapPreview({ session, onOpen }: { session: LearningSession; onOpen: (focus?: MapFocus) => void }) {
  ...
  return <nav className={`knowledge-map-preview ${styles.preview}`} aria-label={t("本题知识脉络")}>
    <button type="button" className={`knowledge-map-preview__open ${styles.open}`} aria-label={t("本题知识图谱")} onClick={() => onOpen(node ? { title: node.title } : undefined)}>
      <BookIcon className="h-4 w-4 shrink-0"/><span>{t("查看完整知识图谱")}</span><ArrowIcon className="ml-auto h-4 w-4 shrink-0"/>
    </button>
  </nav>;
}
